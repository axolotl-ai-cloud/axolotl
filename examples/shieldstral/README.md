...
- Only the answer and `</s>` are trained. Set `train_on_eos: none` on the dataset entry to train the answer token alone. Run `axolotl preprocess config.yaml --debug` to check the masking (text only, `preprocess` refuses configs with `skip_prepare_dataset`).
- Balance `yes` and `no` in your dataset. A skew moves the operating point away from the 0.5 threshold, which is worth recalibrating on a held-out split after training.
- The configs use `sequence_len: 4096` (text) and `2048` (vision). The model was trained on sequences up to 32k, so raise them for longer documents.
- At inference, the model card scores with `temperature=0.0`, `max_tokens=1` and `logprobs=True, top_logprobs=20`, then renormalises the `yes`/`no` probabilities. `top_p` and other sampling settings have no effect since the verdict is read from the logprobs rather than sampled.
...
The sample dataset [Nanobit/text-shieldstral-2k-test](https://huggingface.co/datasets/Nanobit/text-shieldstral-2k-test) is derived from [BeaverTails](https://huggingface.co/datasets/PKU-Alignment/BeaverTails), mapping its `is_safe` flag to the answer. Its labels are noisy, so treat it as a format check rather than a recipe for improving the model.
...
The sample dataset [Nanobit/text-vision-shieldstral-2k-test](https://huggingface.co/datasets/Nanobit/text-vision-shieldstral-2k-test) reuses one benign photo and cycles the `<Query>` over safety questions it does not trigger and factual questions about the scene, so both answers appear without synthesising unsafe imagery. The model already answers these correctly, so the loss starts near zero; it is a format check, not a learning demo.
...
We only support the `mistral-common` tokenizer for Supervised Fine-tuning at the moment and for `type: chat_template` only. Shieldstral ships `tekken.json` and no jinja chat template, so `tokenizer_use_mistral_common: true` is required.
