...
@triton.jit
def _csa_fwd_kernel(
    Q, KS, KC, BB, sinks, Out, L, scale,
    sqb, sqh, sqm, sqd, skb, skn, skd, scb, scn, scd,
    sbb, sbm, sbt, sob, soh, som, sod,
    H, S, T, WINDOW, EL,
    D: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, ACC: tl.constexpr, PREC: tl.constexpr,
):
    ...
    q = tl.load(Q + b * sqb + h * sqh + offs_m[:, None] * sqm + offs_d[None, :] * sqd,
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        acc = acc * alpha[:, None]
        v = tl.load(KS + b * skb + offs_n[:, None] * skn + offs_d[None, :] * skd, mask=nmask[:, None], other=0.0)
    ...
    for start_t in range(0, T, BLOCK_N):
        ...
        tmask = offs_t < T
        kc = tl.load(KC + b * scb + offs_t[:, None] * scn + offs_d[None, :] * scd, mask=tmask[:, None], other=0.0)
        qk = tl.dot(q, tl.trans(kc), input_precision=PREC) * scale
        bb = tl.load(BB + b * sbb + offs_m[:, None] * sbm + offs_t[None, :] * sbt,
                     mask=(offs_m[:, None] < S) & tmask[None, :], other=float("-inf"))
        ...
        l_i = l_i * alpha + tl.sum(p, 1)
        acc = acc * alpha[:, None]
        vc = tl.load(KC + b * scb + offs_t[:, None] * scn + offs_d[None, :] * scd, mask=tmask[:, None], other=0.0)
        acc += tl.dot(p.to(ACC), vc.to(ACC), input_precision=PREC).to(tl.float32)
        m_i = m_new

    acc = acc / l_i[:, None]
    tl.store(Out + b * sob + h * soh + offs_m[:, None] * som + offs_d[None, :] * sod,
             acc.to(Out.dtype.element_ty), mask=offs_m[:, None] < S)
...
def _csa_fwd(q, ks, kc, bb, sinks, scale, window):
    ...
    _csa_fwd_kernel[grid](
        ...
        H, S, T, window, q.element_size(), D=D, ACC=ACC, PREC=PREC)
    return out, L
...
def _csa_bwd_dq_kernel(
    Q, KS, KC, BB, sinks, DO, L, Delta, DQ, DSINK, scale,
    sqb, sqh, sqm, sqd, skb, skn, skd, scb, scn, scd, sbb, sbm, sbt,
    H, S, T, WINDOW, EL,
    D: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, ACC: tl.constexpr, PREC: tl.constexpr,
):
    pid_m = tl.program_id(0)
    ...
    offs_d = tl.arange(0, D)
    mmask = offs_m < S
    q = tl.load(Q + b * sqb + h * sqh + offs_m[:, None] * sqm + offs_d[None, :] * sqd, mask=mmask[:, None], other=0.0)
    do = tl.load(DO + b * sqb + h * sqh + offs_m[:, None] * sqm + offs_d[None, :] * sqd, mask=mmask[:, None], other=0.0)
    ...
    for start_t in range(0, T, BLOCK_N):
        ...
        qk = tl.dot(q, tl.trans(kc), input_precision=PREC) * scale
        bb = tl.load(BB + b * sbb + offs_m[:, None] * sbm + offs_t[None, :] * sbt,
                     mask=mmask[:, None] & tmask[None, :], other=float("-inf"))
    ...
    tl.store(DQ + b * sqb + h * sqh + offs_m[:, None] * sqm + offs_d[None, :] * sqd,
             dq.to(DQ.dtype.element_ty), mask=mmask[:, None])


@triton.autotune(configs=_CONFIGS, key=["H", "EL"], reset_to_zero=["DKS", "DKC"],
                 prune_configs_by={"early_config_prune": _prune_smem(2, 2)})
@triton.jit
def _csa_bwd_dkv_kernel(
    Q, KS, KC, BB, DO, L, Delta, DKS, DKC, scale,
    sqb, sqh, sqm, sqd, skb, skn, skd, scb, scn, scd, sbb, sbm, sbt,
    H, S, T, WINDOW, EL,
    D: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, ACC: tl.constexpr, PREC: tl.constexpr,
):
    ...
    if is_comp:
        ...
        nmask = offs_n < T
        kc = tl.load(KC + b * scb + offs_n[:, None] * scn + offs_d[None, :] * scd, mask=nmask[:, None], other=0.0)
        ...
        for start_m in range(0, S, BLOCK_M):  # every query may attend a compressed key
            ...
            delta = tl.load(Delta + pid_bh * S + offs_m, mask=mmask, other=0.0)
            qk = tl.dot(q, tl.trans(kc), input_precision=PREC) * scale
            bb = tl.load(BB + b * sbb + offs_m[:, None] * sbm + offs_n[None, :] * sbt,
                         mask=mmask[:, None] & nmask[None, :], other=float("-inf"))
            p = tl.where(mmask[:, None] & nmask[None, :], tl.exp(qk + bb - l_i[:, None]), 0.0)
            dp = tl.dot(do, tl.trans(kc), input_precision=PREC)
            ds = (p * (dp - delta[:, None]) * scale).to(ACC)
        ...
        offs_n = start_n + tl.arange(0, BLOCK_N)
        nmask = offs_n < S
        ks = tl.load(KS + b * skb + offs_n[:, None] * skn + offs_d[None, :] * skd, mask=nmask[:, None], other=0.0)
        dk = tl.zeros([BLOCK_N, D], tl.float32)
        dv = tl.zeros([BLOCK_N, D], tl.float32)
...
def _csa_bwd(q, ks, kc, bb, sinks, out, do, L, scale, window):
    ...
    _csa_bwd_dq_kernel[lambda m: (triton.cdiv(S, m["BLOCK_M"]), B * H)](
        q, ks, kc, bb, sinks, do, L, delta, dq, dsink, scale, *qs, *ss, *cs, *bs,
        H, S, T, window, EL, D=D, ACC=ACC, PREC=PREC)
    _csa_bwd_dkv_kernel[lambda m: (triton.cdiv(S, m["BLOCK_N"]) + triton.cdiv(T, m["BLOCK_N"]), B * H)](
        q, ks, kc, bb, do, L, delta, dks, dkc, scale, *qs, *ss, *cs, *bs,
        H, S, T, window, EL, D=D, ACC=ACC, PREC=PREC)
...
class _CSAAttn(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        dq, dks, dkc, dsink = _csa_bwd(q, ks, kc, bb, sinks, out, do, L, ctx.scale, ctx.window)
...
def csa_attn(q, kv_slide, kv_comp, block_bias, sinks, scale=None, sliding_window=128):
    ...
    return _CSAAttn.apply(q.contiguous(), ks, kc, bb, sinks.contiguous(), scale, sliding_window)
