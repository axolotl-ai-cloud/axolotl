...
@triton.jit
def _clamped_swiglu_fwd_kernel(gate_ptr, up_ptr, out_ptr, n_elements, LIMIT, block_size: tl.constexpr):
...
@triton.jit
def _clamped_swiglu_bwd_kernel(grad_out_ptr, gate_ptr, up_ptr, n_elements, LIMIT, block_size: tl.constexpr):
    off = tl.program_id(0) * block_size + tl.arange(0, block_size)
    mask = off < n_elements
...
def _clamped_swiglu_backward(grad_output, gate, up, limit):
    n = grad_output.numel()
    grid = lambda m: (triton.cdiv(n, m["block_size"]),)
    _clamped_swiglu_bwd_kernel[grid](grad_output, gate, up, n, float(limit), block_size=1024)
...
def apply_lora_mlp_clamped_swiglu(self, X, inplace: bool = False):
    ...
    gateW, gateb, gateQ, gateA, gateB, gateS, gateLB, gateDrop, gateMag = get_lora_parameters(self.gate_proj)
    upW, upb, upQ, upA, upB, upS, upLB, upDrop, upMag = get_lora_parameters(self.up_proj)
    downW, downb, downQ, downA, downB, downS, downLB, downDrop, downMag = get_lora_parameters(self.down_proj)
    ...
    return LoRA_MLP.apply(
        X, X_drop,
        gateW, gateb, gateQ, gateA, gateB, gateS, gateLB, gateMag,
        upW, upb, upQ, upA, upB, upS, upLB, upMag,
        downW, downb, downQ, downA, downB, downS, downLB, downMag,
        fwd, bwd, inplace,
    )
...
def patch_dsv4_shared_mlp_lora(model):
    ...
    for module in model.modules():
        if type(module).__name__ != "DeepseekV4MLP":
            continue
        if all(hasattr(getattr(module, p, None), "lora_A") for p in ("gate_proj", "up_proj", "down_proj")):
