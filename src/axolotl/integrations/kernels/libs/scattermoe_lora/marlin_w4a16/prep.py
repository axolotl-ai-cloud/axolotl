...
def marlin_moe_gemm(ext, a, b_qweight, b_scales, global_scale, workspace, sorted_token_ids,
                    expert_ids, num_tokens_past_padded, topk_weights, moe_block_size, top_k,
                    mul_topk_weights, size_m, size_n, size_k, out=None):
    ...
    return ext.moe_wna16_marlin_gemm(
        a, out, b_qweight, None, b_scales, None, global_scale, None, None, None,
        workspace, sorted_token_ids, expert_ids, num_tokens_past_padded, topk_weights,
        moe_block_size, top_k, mul_topk_weights, FLOAT4_E2M1F_ID, size_m, size_n, size_k,
...
def prepare_nvfp4_weight_for_marlin(qdata, block_scale, global_scale, size_n, size_k,
                                    param_dtype, repack_fn):
    ...
    for i in range(E):
        ...
        ms = marlin_permute_scales(s, size_k=size_k, size_n=size_n, group_size=GROUP_SIZE)
        ms, _ = nvfp4_marlin_process_scales(ms, scale_factor=csf, a_dtype=param_dtype)
        sc.append(ms)
    sc = torch.stack(sc, 0)

    g = nvfp4_marlin_process_global_scale(global_scale.to(torch.float32), param_dtype) / csf
