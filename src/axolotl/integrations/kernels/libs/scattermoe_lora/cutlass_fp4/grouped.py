...
def _helpers():
    ...
    return cutlass, cute, cutlass_torch, from_dlpack, sm120_utils, cvt_sf_MKL_to_M32x4xrm_K4xrk_L
...
class GroupedFp4Gemm:
    ...
    def __init__(self, Mt, N, K, E, mode="nvfp4"):
        ...
        from cutlass import Float4E2M1FN, Float32, BFloat16
        ...
        self.sfa_ct, self.sfa_st, self.sfa_g, self.sfa_shape = self._sf(Mt, K, 1, sf_vec, sf_dtype)
        self.sfb_ct, self.sfb_st, self.sfb_g, self.sfb_shape = self._sf(N, K, E, sf_vec, sf_dtype)
        c_ref = cutlass_torch.matrix(1, Mt, N, False, BFloat16)
        self.c_ct, self.c_st = cutlass_torch.cute_tensor_like(c_ref, BFloat16, is_dynamic_layout=True, assumed_align=16)
        self.c_ct.mark_compact_shape_dynamic(mode=1, stride_order=(2, 0, 1), divisibility=1)
        self.c_ct = cutlass_torch.convert_cute_tensor(c_ref.cuda(), self.c_ct, BFloat16, is_dynamic_layout=True)
        ...
        self.compiled = cute.compile(gemm, self.a_ct, self.b_ct, self.sfa_ct, self.sfb_ct,
                                     self.c_ct, self.mi_ct, mac, self.stream)
    ...
    def _operand(self, mn, k, E, op_dtype):
        ...
        ct, st = cutlass_torch.cute_tensor_like(ref, op_dtype, is_dynamic_layout=True, assumed_align=16)
        ct.mark_compact_shape_dynamic(mode=1, stride_order=(2, 0, 1), divisibility=2 if op_dtype is Float4E2M1FN else 1)
        ct = cutlass_torch.convert_cute_tensor(ref.cuda(), ct, op_dtype, is_dynamic_layout=True)
        return ct, st
    ...
    def _sf(self, mn, k, E, sf_vec, sf_dtype):
        ...
        nat = cutlass_torch.create_and_permute_torch_tensor(
            (E, mn, sfk), torch.float64, permute_order=(1, 2, 0), init_type=cutlass_torch.TensorInitType.SKIP)
        nat.copy_(torch.arange(E * mn * sfk, dtype=torch.float64).reshape(E, mn, sfk).permute(1, 2, 0))
        cf = cutlass_torch.create_and_permute_torch_tensor(
            mma_shape, torch.float64, permute_order=(3, 4, 1, 5, 2, 0), init_type=cutlass_torch.TensorInitType.SKIP)
        cvt(from_dlpack(nat), from_dlpack(cf))
        gather = cf.reshape(-1).round().long().cuda()
        cf_e = cutlass_torch.create_and_permute_torch_tensor(
            mma_shape, torch.float32, permute_order=(3, 4, 1, 5, 2, 0), init_type=cutlass_torch.TensorInitType.SKIP)
        ct, st = cutlass_torch.cute_tensor_like(cf_e, sf_dtype, is_dynamic_layout=True, assumed_align=16)
        ct = cutlass_torch.convert_cute_tensor(cf_e.cuda(), ct, sf_dtype, is_dynamic_layout=True)
        return ct, st, gather, tuple(st.shape)

    @staticmethod
    def _inject_operand(st, qdata):
        n = qdata.numel()
        st.view(torch.uint8).permute(2, 0, 1).reshape(-1)[:n].copy_(qdata.reshape(-1).view(torch.uint8))
    ...
    def forward(self, a_q, a_s, m_indices):  # a_q:[1,Mt,*], a_s:[1,Mt,sfk], m_indices:[Mt/128]
        ...
        self.compiled(self.a_ct, self.b_ct, self.sfa_ct, self.sfb_ct, self.c_ct, self.mi_ct, self.stream)
        return self.c_st[:, :, 0]
