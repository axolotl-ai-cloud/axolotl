...
def _qk(X, Q, S, sx0, sx1, sq0, sq1, ss0, ss1, BPM: tl.constexpr):
    ...
    blk = tl.arange(0, BPM)
    e = tl.arange(0, 32)
    k = (b0 + blk)[:, None] * 32 + e[None, :]          # [BPM,32]
    x = tl.load(X + m * sx0 + k * sx1).to(tl.float32)
    amax = tl.max(tl.abs(x), axis=1)                    # [BPM]
    # FLOOR e8m0 scale: 2^(floor(log2(amax)) - floor(log2(448)))
    exp = tl.floor(tl.log2(tl.where(amax > 0, amax, 1.0))) - 8.0   # floor(log2(448))=8
    sc = tl.exp2(exp)                                   # [BPM]
    scb = sc[:, None]
...
def mxfp8_quant(x, bpm=4):
    ...
    S = torch.empty(M, nblk, device=x.device, dtype=torch.uint8)  # e8m0 biased-exponent bytes
