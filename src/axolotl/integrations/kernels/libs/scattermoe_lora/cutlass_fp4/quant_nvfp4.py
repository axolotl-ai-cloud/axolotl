...
def _qk(X, Q, S, sx0, sx1, sq0, sq1, ss0, ss1, BPM: tl.constexpr):
    ...
    sc = sc1d[:, None]
    alo = tl.abs(lo) / sc
    clo = (tl.where(alo > 0.25, 1, 0) + tl.where(alo > 0.75, 1, 0) + tl.where(alo > 1.25, 1, 0)
           + tl.where(alo > 1.75, 1, 0) + tl.where(alo > 2.5, 1, 0) + tl.where(alo > 3.5, 1, 0)
           + tl.where(alo > 5.0, 1, 0)).to(tl.int32) + tl.where(lo < 0, 8, 0).to(tl.int32)
    ahi = tl.abs(hi) / sc
    chi = (tl.where(ahi > 0.25, 1, 0) + tl.where(ahi > 0.75, 1, 0) + tl.where(ahi > 1.25, 1, 0)
           + tl.where(ahi > 1.75, 1, 0) + tl.where(ahi > 2.5, 1, 0) + tl.where(ahi > 3.5, 1, 0)
           + tl.where(ahi > 5.0, 1, 0)).to(tl.int32) + tl.where(hi < 0, 8, 0).to(tl.int32)
...
def nvfp4_quant(x, bpm=8):
    ...
    _qk[(M, triton.cdiv(nblk, bpm))](x, Q, S, x.stride(0), x.stride(1),
                                     Q.stride(0), Q.stride(1), S.stride(0), S.stride(1), BPM=bpm)
    return Q, S
