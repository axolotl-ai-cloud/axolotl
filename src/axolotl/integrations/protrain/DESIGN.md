...
### Single-Stream Allocation (App B.2 — DEFERRED)

**Status: partially implemented — class shipped, runtime wire-up deferred.**

- **Paper's design.** App B.2 specifies that ProTrain routes *every* allocation (chunk gather, prefetch buffers, optimizer state, pinned host shadows) through the *default* CUDA stream. PyTorch's caching allocator keeps a separate free list per stream, so a tensor freed on stream A cannot be reused for an allocation on stream B without `record_stream` hand-holding. Single-streaming the allocations gives the allocator one heap to amortize across all phases and avoids cross-stream dealloc-sync bookkeeping.
- **Current state.** `SingleStreamAllocator` is defined at `runtime/streams.py` as a real, working `__enter__` / `__exit__` / `sync()` context manager — but it has **zero call sites** in production code. `runtime/scheduler.py::_init_streams` does the *opposite* of the paper: it constructs two dedicated non-default `torch.cuda.Stream()` instances (`self._prefetch_stream`, `self._swap_stream`) so PCIe copies overlap the compute stream. Constructing a `SingleStreamAllocator` now emits a one-time `UserWarning` pointing at this section so accidental future callers are discoverable in logs.
- **Why deferred.** Wiring requires routing every chunk-manager + buffer-pool + scheduler allocation through one allocator context, and re-verifying that buffer-pool reuse semantics still hold. Profile data on the project's reference hardware (RTX 3090) has not shown allocator fragmentation as a top-3 contributor to peak — the `α = 1.10` factor (item 1 above) covers it. If a future measurement shows allocator fragmentation costing >5% of peak on a target shape, the work becomes worth doing.
- **Concrete next steps if/when re-prioritized.**
  1. Audit every allocation site (`grep` for `torch.empty`, `torch.zeros`, `torch.empty_like`, `cudaHostAlloc` / `PinnedAllocator`, `BufferPool.acquire`, chunk-manager gather buffers, optimizer-state init).
  2. Refactor each site to run inside `with SingleStreamAllocator(): ...`, passing the stream through scheduler / chunk-manager / buffer-pool boundaries (likely a constructor arg threaded from the runtime root).
  3. Replace the scheduler's dedicated `_prefetch_stream` / `_swap_stream` with stream selection inside the allocator context, or document why those two operations are exempt (the paper does NOT exempt them — re-reading App B.2 closely is part of the work).
  4. Verify `BufferPool` reuse still works: the pool currently relies on PyTorch's per-stream free-list eagerness; under single-stream allocation the reuse path may need an explicit `sync()` before recycling.
  5. Re-run the M1 profiler memory ground-truth and recalibrate `α` downward if fragmentation drops.
