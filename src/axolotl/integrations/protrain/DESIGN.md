...
* **Mode A — GPU-resident / DDP-friendly** (`force_all_persistent=True`). Chosen when the searcher places `n_persist == N_chunk` under the capacity budget — the model fits entirely on GPU and no CPU offload is needed. This is the throughput winner on a non-NVLink 3090 rig: DDP's bucketed NCCL allreduce beats ProTrain's per-param grad sync, and the M7 benchmark measured **3.64x** scaling at world_size=4 on PCIe inter-rank bandwidth.
* **Mode B — replicated CPU-offload** (`zero3_shard=False`). Chosen when the model needs offload AND per-rank CPU RAM can hold the full non-persistent chunk set (`cpu_ram_per_rank >= (N_chunk - n_persist) * S_chunk`). Each rank holds a full replicated copy of every non-persistent chunk; no per-chunk collectives, so it's ~1.9x faster than sharded on a non-NVLink topology.
* **Mode C — ZeRO-3 sharded CPU-offload** (`zero3_shard=True`). Chosen when per-rank CPU RAM is too tight for replication but fits a `1/world_size` shard per chunk. Measured throughput is **0.70x** single-rank on 4x 3090 — the `all_gather` / `reduce_scatter` collectives dominate on a non-NVLink Llama-3B run. Picked only when Mode B can't fit.
...
**Mode B over Mode C — throughput trade-off.** The selector prefers Mode B over Mode C even when C would save pinned RAM, because B is ~1.9x faster on PCIe inter-rank bandwidth and "CPU RAM fits replicated" is the loose binding constraint. Users with binding CPU pressure (e.g., a 96 GB system driving 8 ranks of a model whose non-persistent set is 80 GB replicated but 10 GB sharded) should set `protrain_auto_mode: false, protrain_zero3_shard: true` to force Mode C.
...
**LoRA-adapter grad sync (Path B; `protrain_own_lora_grad_sync`, default true).** Trainable LoRA adapter params are added by PEFT after `protrain_model_wrapper` and sit outside the chunk layout — DDP's bucketed allreduce handles them by default with N small per-bucket collectives (~11 calls/step at LoRA r=16 on a 32-layer 8B model with 7 target modules). ProTrain owns this sync via `_ProTrainOptimizer._sync_lora_grads_path_b()`: trainable LoRA names are registered in `model._ddp_params_and_buffers_to_ignore` during `plugin.post_trainer_create`, then `_sync_lora_grads_path_b` runs once per `optimizer.step()` (before the inner optimizer step) — group grads by dtype, flatten via `_flatten_dense_tensors`, single `dist.all_reduce(op=AVG)` per dtype, unflatten + copy back. Reduces the per-step collective count from 11 (DDP-bucketed) to 1-2 (one per dtype, almost always bf16-only). Bit-equivalent to DDP within FP rounding: 2-rank gloo single-step `torch.allclose(rtol=1e-5, atol=1e-7)` plus 100-step trajectory parity test plus 4-rank NCCL 100-step convergence parity with `|loss_diff| = 0.0` and max weight diff `< 2e-11`. Short-circuits when `world_size <= 1` or no trainable LoRA params discovered. Path B coexists with Mode B/C because chunk-managed params (the per-chunk allreduce's domain) and LoRA params (Path B's domain) are disjoint sets. Opt out with `protrain_own_lora_grad_sync: false` to revert to DDP-bucketed sync.
...
Benchmark: fresh-init Llama-3B + LoRA r=8, bs=2 per rank, seq=256, fp16. 6 iterations per mode, 2 warm-up discarded, median of the remaining 4 is reported. GPUs 1, 4, 5, 7 on a consumer multi-GPU test rig (no NVLink). Reproduce with `CUDA_VISIBLE_DEVICES=1,4,5,7 CUDA_DEVICE_ORDER=PCI_BUS_ID python scripts/benchmark_multi_gpu.py`; the script writes a full JSON dump to `scripts/multi_gpu_benchmark_results.json` (gitignored — generated locally on every run).
...
**How to pick a mode on a non-NVLink 3090 rig.** DDP is the clear throughput winner when the model + optimizer fit on one card (the 7B-LoRA / 3B-full regime) — outer-bucketed NCCL allreduce amortizes better than ProTrain's per-param grad sync and keeps every chunk GPU-resident. Reach for **replicated offload** only when one card can't hold the full model at peak; per-rank GPU drops ~42% (5.4 GB → 3.1 GB here) at a ~3x throughput cost vs DDP. **ZeRO-3 sharded** is only worth it when CPU RAM is the binding constraint — it cuts per-rank pinned CPU by almost exactly `1/world_size` (3.82 GB → 0.96 GB here, a 4.0x reduction, matching world_size) but pays an additional ~1.9x iteration-time penalty from the per-chunk `all_gather` + `reduce_scatter` collectives on PCIe (the per-chunk collective dominance is intrinsic to PCIe vs NVLink, not to a specific PCIe generation). For 7B LoRA on 4x 3090 with NVMe or 128+ GB system RAM, stay on DDP with `force_all_persistent=True`.
