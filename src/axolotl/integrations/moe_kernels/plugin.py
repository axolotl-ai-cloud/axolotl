...
def patch_mixtral_moe_forward_optimized(
    ...
    def moe_forward_optimized(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        sorted_states, sort_indices, inverse_indices, expert_indices = (
            sort_tokens_by_expert(hidden_states, topk_idx, self.top_k)
        )
...
def apply_moe_kernel_patches(
    ...
    **kwargs,
    ...
    if models is None:
        models = ["mixtral", "qwen3_moe", "qwen2_moe"]
    ...
    def pre_model_load(self, cfg: DictDefault):
        ...
        if models_to_patch is None:
            ...
            if "mixtral" in model_type.lower():
                ...
                logger.warning(
                    f"MoE optimization enabled but no compatible model detected for type: {model_type}"
                )
                return
