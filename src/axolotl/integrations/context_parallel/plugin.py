...
class ContextParallelPlugin(BasePlugin):
    ...
    def register(self, cfg: dict):
        ...
        size = (cp or {}).get("size") if isinstance(cp, dict) else getattr(cp, "size", None)
    ...
    def post_model_build(self, cfg, model):
        ...
        if has_recurrent and self._runtime.config.load_balance == rm.LoadBalance.HEAD_TAIL:
            LOG.info("ringmaster: hybrid SSM model — forcing contiguous CP sharding (zigzag permutes tokens, unsafe for the recurrence)")
    ...
    def _wire_recurrent_layers(model):
        ...
        if wiring:
            LOG.info(
                ...
                list(wiring.mamba_modules), wiring.linear_attn_mixers,
            )
        return bool(wiring)
