...
def _lightseq_backward_varlen(
    do, q, k, v, o, L, sm_scale, comm_mode, backward_engine, cu_seq_lens, max_seqlen
):
    BLOCK = 128
    ...
    for time_step in range(0, get_sequence_parallel_size() // 2 + 1):
        ...
        if is_compute_for_local_query(time_step):
            if time_step == 0:
                ...
                ), "We haven't supportted varlen feature in xformer"
    ...
    dq, dk, dv = [
        rearrange(_x, "(b s) h d -> b h s d", s=max_seqlen) for _x in [dq, dk, dv]
    ]
    return dq, dk, dv
...
class _attention_varlen(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale):
        ...
        q, k, v, o, L, cu_seq_lens, max_seqlen = _lightseq_forward_varlen(
            q, k, v, causal, sm_scale, comm_mode
        )
    ...
    def backward(ctx, do):
        ...
        dq, dk, dv = _lightseq_backward_varlen(
            do,
            q,
            k,
            v,
...
def test_op(Z, H, N_CTX, D_HEAD, causal, dtype=torch.bfloat16):
    ...
    q = (
        torch.empty((Z, H, N_CTX - PAD, D_HEAD), dtype=dtype, device="cuda")
    ...
    assert torch.allclose(
        ...
        atol=1e-2,
        rtol=0,
    ), f"rank {rank} fails backward dq"  # {ref_dq[:, :, rank * seq_per_rank: (rank + 1) * seq_per_rank, :]} {tri_dq} {torch.max(ref_dq[:, :, rank * seq_per_rank: (rank + 1) * seq_per_rank, :] - tri_dq)} rank {rank} fails backward dk"
    assert torch.allclose(
