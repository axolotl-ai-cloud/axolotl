...
class ProcessingStrategy:
    ...
    def __init__(
        ...
        """
        Initialize the processing strategy with processor-specific tokenizer and image configuration.
        
        Parameters:
            processor (ProcessorMixin): Processor instance that provides tokenization and optional image token metadata.
            chat_template (Optional[str]): Optional chat template identifier or content used for formatting messages.
            image_size (int | tuple[int, int] | None): Target image size for resizing. If an int is provided, images are treated as square (width=height). If a tuple, interpreted as (width, height). If None, images are not resized.
            image_resize_algorithm (Resampling | None): Pillow resampling algorithm to use when resizing images; defaults to bilinear if omitted.
        
        Notes:
            - The instance field `supports_multi_images` defaults to False and should be set to True by subclasses that accept multiple input images.
            - If the provided processor exposes an `image_token`, the corresponding token id is retrieved from the processor's tokenizer and stored for downstream label masking and input construction.
        """
    ...
    def __call__(self, examples: list[dict]) -> list[dict]:
        """
        Normalize and enrich conversation examples into OpenAI-like message structures with multimedia content.
        
        Converts legacy `conversations` entries to `messages`, normalizes each message's content into a list of typed content blocks (e.g., text, image), optionally deserializes JSON-encoded message content for processors that support it, and attaches image data to message content. When supported by the strategy, multiple images are preserved and mapped to explicit image placeholders or appended to the first user message; otherwise the first image is used and additional images are ignored. Images will be resized when an image size is configured.
        
        Parameters:
            examples (list[dict]): Conversation records, each containing either a `messages` key (OpenAI-style) or a legacy `conversations` key.
        
        Returns:
            list[dict]: Processed examples in OpenAI format with a `messages` key whose entries contain typed `content` blocks and any attached/embedded image objects.
        ...
        def convert_legacy_format(example: dict) -> dict:
            """
            Convert a legacy example using the 'conversations' key into an OpenAI-style example using the 'messages' key.
            
            Converts each conversation entry {"from": str, "value": Any} into a message {"role": <normalized role>, "content": <value>}, removes the original 'conversations' key, and returns a deep-copied example with the new 'messages' list.
            
            Parameters:
                example (dict): Input example expected to contain a 'conversations' list of dicts, each with keys 'from' and 'value'.
            
            Returns:
                dict: A deep copy of the input example with 'conversations' removed and 'messages' added, where each message has 'role' and 'content'.
            """
            messages = [
        ...
        def convert_messages_to_multimedia_messages(messages: list[dict], is_qwen2_vl: bool = False) -> list[dict]:
            """
            Normalize message entries into multimodal message objects with explicit content types.
            
            Parameters:
                messages (list[dict]): Sequence of message objects each containing at least a "role" and a "content" field.
                    "content" may be a plain string or a list of content blocks.
                is_qwen2_vl (bool): When True, attempt to parse string content that begins with '[' or '{' as JSON;
                    if parsing succeeds the resulting Python structure is used, otherwise the original string is kept.
            
            Returns:
                list[dict]: A new list of messages where each message is a dict with "role" and "content".
                    If an input message's content was a string, it is converted to a single content block
                    [{"type": "text", "text": <string>}]. If the input content was already a list, it is preserved.
            """
...
class Qwen2VLProcessingStrategy(ProcessingStrategy):
    ...
    def __init__(
        ...
        """
        Initialize a processing strategy configured for Qwen2-VL models.
        
        Parameters:
            processor (ProcessorMixin): Tokenizer/processor used to convert and encode inputs.
            chat_template (Optional[str]): Optional chat template identifier or content.
            image_size (int | tuple[int, int] | None): Target image size (square int or (width, height)) or None to keep original.
            image_resize_algorithm (Resampling | None): Pillow resampling algorithm to use when resizing images.
        
        Description:
            Enables multi-image support for Qwen2-VL, sets the image placeholder token ("<|image_pad|>"), and computes its token ID using the provided processor's tokenizer.
        """
