...
class UserDefinedPrompterType(BaseModel):
    ...
    format: str | None = Field(
        default=None,
        json_schema_extra={"description": "Customizable to be single line or multi-line. Use {instruction}/{input} as key to be replaced. 'format' can include {input}"}
    )
    ...
    field: str | None = Field(
        default=None,
        json_schema_extra={"description": "For `completion` datsets only, uses the provided field instead of `text` column"}
    )
...
class SFTDataset(BaseModel):
    ...
    path: str | None = Field(
        default=None,
        json_schema_extra={"description": "HuggingFace dataset repo | s3:// | gs:// | path to local file or directory"}
    )
    ...
    type: str | UserDefinedPrompterType | None = Field(
        default=None,
        json_schema_extra={"description": "The type of prompt to use for training. [alpaca, gpteacher, oasst, reflection]"}
    )
    input_transform: str | None = None
    shards: int | None = Field(
        default=None,
        json_schema_extra={"description": "split dataset into N pieces (use with shards_idx)"}
    )
    ...
    preprocess_shards: int | None = Field(
        default=None,
        json_schema_extra={"description": "process dataset in N sequential chunks for memory efficiency (exclusive with `shards`)"}
    )
    ...
    chat_template_jinja: str | None = Field(
        default=None,
        json_schema_extra={"description": "Custom jinja chat template. Used only if `chat_template: jinja` or empty."}
    )
    data_files: str | list[str] | None = Field(
        default=None,
        json_schema_extra={"description": "path to source data files"}
    )
    ...
    field_messages: str | None = Field(
        default=None,
        json_schema_extra={"description": "Key containing the messages (default: \"messages\")"}
    )
    field_tools: str | None = Field(
        default=None,
        json_schema_extra={"description": "Key containing the tools (default: \"tools\"). Must be a list[dict] and follow [JSON schema](https://json-schema.org/learn/getting-started-step-by-step)."}
    )
    ...
    message_field_training: str | None = Field(
        default=None,
        json_schema_extra={"description": "The key in the message turn that indicates via boolean whether tokens of a turn should be considered for training. Useful to selectively train on certain turns besides the `roles_to_train`."}
    )
    ...
    split_thinking: bool | None = Field(
        default=None,
        json_schema_extra={"description": "(for Qwen3 template only) Whether to split the assistant content based on a reasoning trace inside delimited tags"}
    )
    logprobs_field: str | None = None
    temperature: float | None = None
    roles_to_train: list[str] | None = Field(
        default=None,
        json_schema_extra={"description": "Roles to train on. The tokens from these roles will be considered for the loss."}
    )
    train_on_eos: str | None = Field(
        default=None,
        json_schema_extra={"description": "Which EOS tokens to train on in the conversation. Possible values are: all: train on all EOS tokens, turn (default): train on the EOS token at the end of each trainable turn, last: train on the last EOS token in the conversation"}
    )
    roles: dict[str, list[str]] | None = Field(
        default=None,
        json_schema_extra={"description": "Roles mapping in the messages. The format is {target_role: [source_roles]}. All source roles will be mapped to the target role. The default is: user: [\"human\", \"user\"], assistant: [\"gpt\", \"assistant\"], system: [\"system\"], tool: [\"tool\"]"}
    )
    drop_system_message: bool | None = Field(
        default=None,
        json_schema_extra={"description": "Whether to drop the system turn from the dataset. Only works with chat_template. This does not drop the default system message from chat_template if it exists. If you wish to, we recommend using a custom jinja template with the default system message removed or adding a system turn with empty content."}
    )
    ...
    revision: str | None = Field(
        default=None,
        json_schema_extra={"description": "The specific revision of the dataset to use when loading from the Hugging Face Hub. This can be a commit hash, tag, or branch name. If not specified, the latest version will be used. This parameter is ignored for local datasets."}
    )
