...
class ModelLoader:
    ...
    def load_model(self) -> Tuple[PreTrainedModel, Optional[PeftConfig]]:
        ...
        if (
            self.cfg.ddp
            and not ("load_in_8bit" in self.model_kwargs and self.model_kwargs["load_in_8bit"])
            and not (self.cfg.rl and "load_in_4bit" in self.model_kwargs and self.model_kwargs["load_in_4bit"])
