...
class ModelLoader:
    ...
    def set_device_map_config(self) -> None:
        ...
        if "mps" in self.device.__str__():
            self.model_kwargs["device_map"] = "mps:0"
        elif "npu" in self.device.__str__():
    ...
    def set_attention_config(self) -> None:
        ...
        if self.cfg.flash_attention:
            ...
            self.model_config._attn_implementation = (
                "sdpa"  # pylint: disable=protected-access
            )
        elif self.cfg.eager_attention:
            self.model_kwargs["attn_implementation"] = "eager"
            self.model_config._attn_implementation = (
                "eager"  # pylint: disable=protected-access
            )
    ...
    def build_model(self, qlora_fsdp) -> bool:
        ...
        if (  # pylint: disable=condition-evals-to-constant)
            ...
            if (
                hasattr(self.model_config, "max_seq_len")
                and self.model_config.max_seq_len
                and self.cfg.sequence_len > self.model_config.max_seq_len
            ):
                self.model_config.max_seq_len = self.cfg.sequence_len
                LOG.warning(f"increasing context length to {self.cfg.sequence_len}")
            elif (
                hasattr(self.model_config, "max_sequence_length")
                and self.model_config.max_sequence_length
                and self.cfg.sequence_len > self.model_config.max_sequence_length
            ):
                self.model_config.max_sequence_length = self.cfg.sequence_len
                LOG.warning(f"increasing context length to {self.cfg.sequence_len}")
            if self.cfg.gptq:
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.base_model,
    ...
    def load_model(self) -> Tuple[PreTrainedModel, Optional[PeftConfig]]:
        ...
        if (
            self.cfg.ddp
            and not self.model_kwargs["load_in_8bit"]
            and not (self.cfg.rl and self.model_kwargs["load_in_4bit"])
            and not skip_move_to_device
        ):
            # TODO revaldate this conditional
            self.model.to(f"{self.device.__str__()}:{self.cfg.local_rank}")
...
def load_model(
    ...
    """
    loader = ModelLoader(cfg, tokenizer, inference, reference_model)
