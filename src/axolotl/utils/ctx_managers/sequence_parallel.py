    ...
    for key in batch:
        if (
            ...
            if ring_attn_func in [
                ...
                if key == "input_ids":
                    print(
                        f"{dist.get_rank()}: batch[key].shape before: {batch[key].shape}"
                    )
                ...
                if key == "input_ids":
                    print(
                        f"{dist.get_rank()}: batch[key].shape after: {batch[key].shape}"
                    )
...
class AllGatherWithGrad(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ring_attn_func in [RingAttnFunc.VARLEN_LLAMA3, RingAttnFunc.BATCH_RING]:
            ...
            if grad_input.shape != input_shape:
                raise ValueError(
                    f"Gradient shape mismatch in stripe backward: "
                    f"got {grad_input.shape} but expected {input_shape}"
                )
...
class SequenceParallelContextManager:
    ...
    def gather_tensor(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            gathered_tensors = [
                torch.zeros_like(tensor.detach()) for _ in range(world_size)
            ]
