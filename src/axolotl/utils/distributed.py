...
def gather_scalar_from_all_ranks(fn, world_size=1):  # pylint: disable=invalid-name
    ...
    if not is_distributed():
        return [value_scalar]
    value_tensor = torch.tensor(value_scalar, device=torch.cuda.current_device()).float()
...
def compute_and_broadcast(fn):  # pylint: disable=invalid-name
    ...
    if is_main_process():
        ...
        value_tensor = torch.tensor(0.0, device=torch.cuda.current_device())  # Placeholder tensor
