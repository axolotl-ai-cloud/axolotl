"""Fused q_norm/k_norm + RoPE for Qwen3.5 (gated q_proj, ``unit_offset=True`` RMSNorm)."""
...
def patch_qwen3_5_fused_attn() -> None:
    from transformers.models.qwen3_5.modeling_qwen3_5 import Qwen3_5Attention

    if getattr(Qwen3_5Attention, "_axolotl_fused_attn_patched", False):
        return

    Qwen3_5Attention.forward = _make_fused_forward()
    Qwen3_5Attention._axolotl_fused_attn_patched = True
    logger.info(
        "Patched Qwen3_5Attention.forward with fused RMSNorm+RoPE Triton kernel"
    )
