"""Sample-packing and context-parallelism patch for FalconH1 (Mamba2/Attention hybrid).
...
Key structural difference from NemotronH:
- FalconH1 runs SSM and attention IN PARALLEL every block (no block_type dispatch).
  seq_idx is only passed to the SSM (mamba) path; attention is unaffected.
- The fused fast path (mamba_split_conv1d_scan_combined) uses return_final_states=False
  so it returns no ssm_state — it is gated off when seq_idx is set.
- time_step is already post-softplus when passed to mamba_chunk_scan_combined
  (softplus applied explicitly before the kernel call), unlike some other models.
"""
...
def patch_falcon_h1_modeling_packing():
    """Patch FalconH1 for sample packing: seq_idx threading into Mamba2 SSM kernels.

    _get_unpad_data is handled by SUPPORTED_MULTIPACK_MODEL_TYPES / patch_for_multipack().
    This function only applies the seq_idx patches that are unique to falcon_h1.
    """
    try:
    ...
    # causal_conv1d_fn and mamba_chunk_scan_combined.
    # Fused fast path (mamba_split_conv1d_scan_combined) is bypassed when
    # seq_idx is set because it uses return_final_states=False — no ssm_state
    # is returned, so CP correction would be impossible.
    def patched_cuda_kernels_forward(
        ...
        if use_precomputed_states:
            ...
            hidden_states = mod.selective_state_update(
                ...
                C,
                D,
                z=gate.view(batch_size, self.num_heads, self.head_dim)
            ...
            # Fused fast path: gate off when seq_idx is set (return_final_states=False
            # means no ssm_state is returned, making CP correction impossible) or
            # when we're in eval / have a cache.
            if self.training and cache_params is None and seq_idx is None:
                ...
                # time_step is post-softplus (applied explicitly here, not inside kernel)
    ...
    # Patch 2: FalconH1Mixer.forward — add seq_idx param, pass through to both
    # cuda and torch paths.
    def patched_mixer_forward(
        ...
        if seq_idx is not None and mod.causal_conv1d_fn is None:
            raise RuntimeError(
                "FalconH1 sample packing requires causal_conv1d_fn. "
    ...
    # Patch 3: FalconH1DecoderLayer.forward — compute seq_idx from position_ids
    # and pass it only to the SSM (mamba) path. Attention is unaffected.
    # Skipped during single-token decode (has_previous_state=True).
    def patched_decoder_layer_forward(
        ...
        **kwargs,
        ...
        # Compute seq_idx once for all Mamba layers in this block.
        # Skip during decode: has_previous_state=True means single-token generation.
    ...
    FalconH1DecoderLayer.forward = patched_decoder_layer_forward

    LOG.info("Applied FalconH1 sample packing patch (seq_idx threading into Mamba2)")
