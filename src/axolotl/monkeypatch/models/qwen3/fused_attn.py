"""Fuse ``q_norm/k_norm`` + RoPE in ``Qwen3Attention.forward`` via one Triton kernel."""
...
def patch_qwen3_fused_attn() -> None:
    from transformers.models.qwen3.modeling_qwen3 import Qwen3Attention

    if getattr(Qwen3Attention, "_axolotl_fused_attn_patched", False):
        return

    Qwen3Attention.forward = _make_fused_forward()
    Qwen3Attention._axolotl_fused_attn_patched = True
    logger.info("Patched Qwen3Attention.forward with fused RMSNorm+RoPE Triton kernel")
