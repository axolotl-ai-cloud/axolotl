"""Qwen3.5-MoE variant of the qwen3_5 fused-attention monkeypatch."""
    ...
    def fused_forward(
        ...
        **kwargs,
        ...
        query_states = fused_rms_norm_rope(
            query_states, q_w, cos, sin, eps=eps, unit_offset=True
        ).transpose(1, 2)
        key_states = fused_rms_norm_rope(
            key_states, k_w, cos, sin, eps=eps, unit_offset=True
        ).transpose(1, 2)
...
def patch_qwen3_5_moe_fused_attn() -> None:
    from transformers.models.qwen3_5_moe.modeling_qwen3_5_moe import (
        Qwen3_5MoeAttention,
    )

    if getattr(Qwen3_5MoeAttention, "_axolotl_fused_attn_patched", False):
        return

    Qwen3_5MoeAttention.forward = _make_fused_forward()
    Qwen3_5MoeAttention._axolotl_fused_attn_patched = True
    logger.info(
        "Patched Qwen3_5MoeAttention.forward with fused RMSNorm+RoPE Triton kernel"
    )
