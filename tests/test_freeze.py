...
class TestFreezeLayersExcept(unittest.TestCase):
    ...
    def test_freeze_layers_with_dots_in_name(self):
        ...
        self.assertTrue(self.model.features.layer.weight.requires_grad, "model.features.layer should be trainable.")
        self.assertFalse(self.model.classifier.weight.requires_grad, "model.classifier should be frozen.")

    def test_freeze_layers_without_dots_in_name(self):
        freeze_layers_except(self.model, ["classifier"])
        self.assertFalse(self.model.features.layer.weight.requires_grad, "model.features.layer should be trainable.")
        self.assertTrue(self.model.classifier.weight.requires_grad, "model.classifier should be frozen.")

    def test_all_layers_frozen(self):
        freeze_layers_except(self.model, [])
        self.assertFalse(self.model.features.layer.weight.requires_grad, "model.features.layer should be frozen.")
        self.assertFalse(self.model.classifier.weight.requires_grad, "model.classifier should be frozen.")

    def test_all_layers_unfrozen(self):
        freeze_layers_except(self.model, ["features.layer", "classifier"])
        self.assertTrue(self.model.features.layer.weight.requires_grad, "model.features.layer should be trainable.")
        self.assertTrue(self.model.classifier.weight.requires_grad, "model.classifier should be trainable.")
    ...
    def test_freeze_layers_with_range_pattern_start_end(self):
        ...
        self._assert_gradient_output([ZERO, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ZERO, ZERO, ZERO, ZERO, ZERO])
    ...
    def test_freeze_layers_with_range_pattern_single_index(self):
        ...
        self._assert_gradient_output([ZERO, ZERO, ZERO, ZERO, ZERO, ONE_TO_TEN, ZERO, ZERO, ZERO, ZERO])
    ...
    def test_freeze_layers_with_range_pattern_start_omitted(self):
        ...
        self._assert_gradient_output([ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ZERO, ZERO, ZERO, ZERO, ZERO])
    ...
    def test_freeze_layers_with_range_pattern_end_omitted(self):
        ...
        self._assert_gradient_output([ZERO, ZERO, ZERO, ZERO, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN])
    ...
    def test_freeze_layers_with_range_pattern_merge_intersect(self):
        ...
        self._assert_gradient_output([ZERO, ZERO, ZERO, ZERO, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ONE_TO_TEN, ZERO, ZERO])

    def test_freeze_layers_with_range_pattern_merge_separate(self):
        freeze_layers_except(self.model, ["features.layer[1:2]", "features.layer[3:4]", "features.layer[5:6]"])
        self.assertTrue(self.model.features.layer.weight.requires_grad, "model.features.layer should be trainable.")
        self.assertFalse(self.model.classifier.weight.requires_grad, "model.classifier should be frozen.")

        self._assert_gradient_output([ZERO, ONE_TO_TEN, ZERO, ONE_TO_TEN, ZERO, ONE_TO_TEN, ZERO, ZERO, ZERO, ZERO])
    ...
    def _assert_gradient_output(self, expected):
        ...
        torch.testing.assert_close(self.model.features.layer.weight.grad, expected_grads)


class _SubLayerModule(nn.Module):
    def __init__(self):
        super(_SubLayerModule, self).__init__()
