...
class TestAdapterMergeUnmerge:
    ...
    def create_mock_base_model(self, vocab_size=1000, hidden_size=256):
        ...
        mock_model.q_proj.weight = torch.randn(hidden_size, hidden_size, dtype=self.dtype)
        mock_model.q_proj.bias = torch.randn(hidden_size, dtype=self.dtype)
        
        mock_model.v_proj = Mock()
        mock_model.v_proj.weight = torch.randn(hidden_size, hidden_size, dtype=self.dtype)
    ...
    def create_mock_lora_model(self, base_model, r=8, alpha=16):
        ...
        self.lora_A_q = torch.randn(r, base_model.q_proj.weight.shape[1], dtype=self.dtype)
        self.lora_B_q = torch.randn(base_model.q_proj.weight.shape[0], r, dtype=self.dtype)
        
        self.lora_A_v = torch.randn(r, base_model.v_proj.weight.shape[1], dtype=self.dtype)
        self.lora_B_v = torch.randn(base_model.v_proj.weight.shape[0], r, dtype=self.dtype)
