...
def test_fp8_recipe_cli_option_is_generated():
    checked_in_option = next(
        option
        for option in AXOLOTL_CONFIG_CLI_OPTIONS
        if option[0] == ("--fp8-recipe",)
    )
    generated_option = next(
        option for option in generate_options() if option[0] == ("--fp8-recipe",)
    )

    assert checked_in_option == generated_option
    assert checked_in_option[1] is None
