...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "M6C-fix-{1,2,3,4,5,6} now cover EVERY transition window of "
        ...
        "tests/protrain/test_late_nccl_search_skip.py), and M6C-fix-6 "
        ...
        "through during the LoRA container's autograd lifecycle).\n"
        "\n"
        ...
        "    test_bnb_offload.py).\n"
        "\n"
        "Closing this requires either invasive PEFT-internal "
        "instrumentation (untraceable from this codebase) or upstream "
        "PyTorch-side investigation of how at::Tensor::sym_sizes() "
        "captures shape at autograd Node construction. Larger scope "
        "than the M6C-fix-* file-partition framework supports. "
        "Tracked."
    ),
)
...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "M6C-fix-{1,2,3,4,5,6} cover every M6C runtime gather path we "
        "can identify, including the M6C-fix-6 per-container POST-fwd "
        "and POST-bwd defense-in-depth re-binds added on top of "
        "M6C-fix-3's pre-edge pair. All verified at single-GPU + "
        "multi-rank gloo unit scope (test_lora_offload_mode.py and "
        "test_sharded_lora_offload.py). The 4-rank Phase 1 (Mode C "
        "train+save) still fails at iter-0 backward with the same "
        "shape-mismatch class as the A→C direction.\n"
        "\n"
        ...
        "the M6C-fix-7+ candidate root causes. Tracked for a follow-up "
        "dispatch outside the M6C-fix-* file-partition framework. "
        "Workaround: use Mode A (all-persistent, no offload) for "
        "plain-LoRA multi-rank runs, or bnb-quantized Mode C "
        "(test_bnb_offload.py covers that path)."
    ),
)
