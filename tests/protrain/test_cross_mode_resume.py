...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "M6C-fix-{1,2,3,4,5} now cover EVERY M6C runtime gather path we "
        "can identify: M6C-fix-1 the cross-mode resume hook in "
        "plugin.py, M6C-fix-2 the per-PEFT-LoRA-container gather in "
        "profiler/on_demand.py, M6C-fix-3 the per-container fwd/bwd "
        "pre-gather hooks in runtime/hooks.py, M6C-fix-4 routes "
        "Scheduler.ensure_chunks_resident SYNCHRONOUSLY through the "
        "chunk manager (instead of via the prefetch stream) so the "
        "LoRA factor's param.data rebind happens on the same logical "
        "execution stream the autograd op consumes the shape from, and "
        "M6C-fix-5 unblocks the late-NCCL re-search RuntimeError on "
        "explicit-override paths (so multi-GPU Mode C with explicit "
        "n_persist/n_buffer/n_swap/n_checkpoint overrides actually "
        "REACHES the iter-0 backward instead of bailing inside "
        "post_trainer_create — pinned by "
        ...
        "passes. Despite all five fixes, the 4×3090 multi-GPU sharded "
        ...
        "\n"
        "M6C-fix-5 empirical findings (4×3090 rig with peft_autocast_"
        "adapter_dtype: false workaround applied):\n"
        "  * Pre-workaround failure mode: 'ToCopyBackward0 returned an "
        "    invalid gradient at index 0 - got [14336, 16] but expected "
        "    shape compatible with [0]' (the bf16 autocast cast op was "
        "    the autograd-recorded source-shape consumer).\n"
        "  * Post-workaround failure mode: 'TBackward0 returned an "
        "    invalid gradient at index 0 - got [14336, 16] but expected "
        "    shape compatible with [0]' — the autocast _to_copy op is "
        "    eliminated (workaround works for the autocast layer), but "
        "    the next-deeper autograd op in the chain (the implicit "
        "    transpose inside torch.nn.functional.linear's at::linear "
        "    decomposition: input @ weight.t()) takes its place. The "
        "    weight is still recorded against its [0] empty-placeholder "
        "    size at the moment at::linear dispatches.\n"
        "\n"
        "Construction site of the residual TBackward0 op:\n"
        "  - peft/tuners/lora/layer.py:969 — "
        "    `result = result + lora_B(lora_A(dropout(x))) * scaling`. "
        "    `lora_B` and `lora_A` are nn.Linear children inside the "
        "    OUTER lora.Linear container (e.g. q_proj/v_proj/down_proj). "
        "    The inner `lora_B.forward(...)` calls "
        "    torch.nn.functional.linear, which dispatches to at::linear "
        "    and decomposes to `input @ weight.t()`. The implicit `.t()` "
        "    creates a TBackward0 graph node bound to weight's `.size()` "
        "    at construction time.\n"
        "  - The OUTER lora.Linear container HAS a registered "
        "    pre-forward hook (M6C-fix-3, runtime/hooks.py:372) that "
        "    calls `ensure_chunks_resident(chunk_ids)` covering every "
        "    descendant param. With M6C-fix-4 this routes synchronously "
        "    through `chunk_manager.gather`, so the gather completes "
        "    before the inner Linear's forward dispatches. The "
        "    pre-backward analog (runtime/hooks.py:378) likewise "
        "    re-gathers before the container's backward starts.\n"
        "  - Despite the gather firing, TBackward0 fails the size "
        "    check. Hypothesis: the pre-backward hook on the OUTER "
        "    container fires before the OUTER module's backward begins, "
        "    but the inner lora_B/lora_A nn.Linear children's backward "
        "    autograd ops execute as part of the SAME backward pass — "
        "    AFTER the container's pre-backward hook fires they execute, "
        "    but BEFORE the chunk's release. The release happens at the "
        "    block-level post-backward (runtime/hooks.py:284). So the "
        "    chunk should still be resident at TBackward0 apply time. "
        "    Yet the size check reads `[0]` — suggests param.data was "
        "    re-released by some intervening path (post-forward that we "
        "    do NOT install on containers? a separate scheduler "
        "    reentrancy?) before the backward chain reaches the inner "
        "    Linear.\n"
        "\n"
        "Recommended next step (out of M6C-fix-5 scope; tracked for a "
        "follow-up dispatch): a 4-rank torch anomaly-mode trace bound "
        "to the inner lora_B nn.Linear's forward dispatch, capturing "
        "the autograd-graph-recording call site AND every chunk_manager "
        "gather/release entry point that fires between the OUTER "
        "container pre-forward hook and the inner backward apply. The "
        "specific question to answer: does the chunk get released "
        "between the OUTER lora.Linear post-forward (no hook) and the "
        "inner TBackward0 apply? If yes, install a per-container "
        "post-backward hook to keep the chunk resident through the "
        "inner-op tail. If no, the gather is firing but the rebind isn't "
        "propagating through the inner Linear's weight reference — that "
        "would require investigating whether nn.Linear caches its "
        "weight tensor identity at module construction (it does — "
        "self.weight is a Parameter; rebinding param.data should be "
        "transparent, BUT autograd's graph recording may have captured "
        "the old data_ptr).\n"
        "\n"
        "Closing this is a known-larger scope than the M6C-fix-* file-"
...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "M6C-fix-{1,2,3,4,5} cover every M6C runtime gather path we can "
        "identify (cross-mode resume hook, profiler container gather, "
        "runtime container hooks, synchronous Scheduler routing, late-"
        "NCCL re-search override-skip). All verified at single-GPU + "
        "multi-rank gloo unit scope. The 4-rank Phase 1 (Mode C "
        "train+save) still fails at iter-0 backward with the same "
        "shape-mismatch class as the A→C direction: the M6C-fix-5 "
        "empirical run on the A→C path with peft_autocast_adapter_dtype: "
        "false applied confirmed the failure mode shifts from "
        "'ToCopyBackward0 ... shape compatible with [0]' to "
        "'TBackward0 ... shape compatible with [0]' — the autocast "
        "workaround eliminates the _to_copy op but the next-deeper "
        "autograd op (the implicit transpose inside "
        "F.linear(input, weight) → input @ weight.t()) takes its place. "
        "C→A Phase 1 was NOT empirically retried after M6C-fix-5 per the "
        "safety protocol (one multi-GPU attempt per direction max; the "
        "A→C run showed the deeper construction-site gap is symmetric "
        "and would manifest the same way here at the inner lora_A/lora_B "
        "nn.Linear forward dispatch). See the A→C xfail reason for the "
        "full construction-site analysis (peft/tuners/lora/layer.py:969 "
        "→ at::linear → implicit .t()) and the recommended anomaly-mode "
        "follow-up. Tracked for a follow-up dispatch outside the M6C-"
        "fix-* file-partition framework."
    ),
)
