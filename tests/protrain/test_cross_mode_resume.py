...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "M6C-fix-3 (per-PEFT-LoRA-container pre-gather hooks in "
        "runtime/hooks.py) closes the unit-scale Mode C training-time "
        "PEFT-LoRA gap (verified by "
        "test_lora_offload_mode.py::test_runtime_lora_e2e_under_offload_mode_smoke "
        "passing single-GPU). The 4×3090 multi-GPU sharded path "
        "(zero3_shard=True) still surfaces the canonical "
        "'ToCopyBackward0 returned an invalid gradient at index 0 - got "
        "[14336, 16] but expected shape compatible with [0]' at iter-0 "
        "loss.backward() of Mode C training of Llama-3-8B + LoRA. "
        "M6C-fix-3 confirms 224 PEFT-LoRA containers are detected and "
        "per-container fwd/bwd pre-gather hooks are installed (log line "
        "'install_hooks (M6C-fix-3): 224 PEFT-LoRA container(s) detected'), "
        "but the recorded autograd source-shape on the bf16 cast remains "
        "[0] — indicating the cast is recorded against a still-released "
        "weight in some sharded-mode-only code path the per-container "
        "synchronous gather doesn't cover. Closing this gap requires "
        "deeper investigation of the zero3_shard gather sequence vs. "
        "autocast op-recording timing — likely needs touching "
        "chunk/manager.py::_gather_sharded (out of M6C-fix-3 scope per "
        "the spec's file partition). Tracked for a follow-up runtime fix."
    ),
)
...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "M6C-fix-3 partially closes the runtime PEFT-LoRA gap: "
        "container detection + per-container fwd/bwd pre-gather hooks "
        "in runtime/hooks.py + ensure_chunks_resident in "
        "runtime/scheduler.py confirmed firing on Llama-3-8B + LoRA "
        "(224 containers detected per log) and verified single-GPU "
        "via test_runtime_lora_e2e_under_offload_mode_smoke. Phase 1 "
        "(Mode C train+save) still fails at iter-0 backward with the "
        "canonical 'ToCopyBackward0 ... shape compatible with [0]' "
        "under the 4-rank zero3_shard path. The remaining gap is "
        "specific to the sharded-gather + autocast op-recording "
        "interaction and likely requires touching chunk/manager.py to "
        "ensure the LoRA-factor sub-chunk's typed-view rebind happens "
        "before autocast records the bf16 cast — out of M6C-fix-3's "
        "MAY-edit scope (runtime/* + this test file only). Tracked "
        "for a follow-up runtime fix."
    ),
)
