...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "M6C-fix-{1,2,3,4} now cover ALL of the M6C runtime gather paths "
        "we can identify: M6C-fix-1 the cross-mode resume hook in "
        "plugin.py, M6C-fix-2 the per-PEFT-LoRA-container gather in "
        "profiler/on_demand.py, M6C-fix-3 the per-container fwd/bwd "
        "pre-gather hooks in runtime/hooks.py, and M6C-fix-4 routes "
        ...
        "execution stream the autograd op consumes the shape from. "
        ...
        "passes. Despite all four fixes, the 4×3090 multi-GPU sharded "
        "path (zero3_shard=True + Llama-3-8B + LoRA) still surfaces "
        "the canonical 'ToCopyBackward0 returned an invalid gradient "
        "at index 0 - got [14336, 16] but expected shape compatible "
        "with [0]' at iter-0 backward of the resumed Mode C training. "
        "Per the M6C-fix-4 attempt (verified empirically against the "
        "real 4×3090 rig), the failure persists with 224 PEFT-LoRA "
        "containers detected and synchronous container-hook routing "
        "in place — indicating the source-shape [0] is recorded by an "
        "autograd op constructed against a code path neither the "
        "block-level nor container-level gather hook covers. Suspected "
        "remaining gap: a sharded-mode-only autograd graph node "
        "(possibly inside DDP gradient reduction registration, "
        "accelerate's autocast-cache priming, or a PEFT internal that "
        "captures the LoRA factor reference at .to(dtype) construction "
        "time before any pre-forward hook fires) that holds a stale "
        "reference to the empty-placeholder weight. Closing this likely "
        "requires either (a) a torch anomaly-mode trace from a 4-rank "
        "live run to identify the exact construction site, or (b) a "
        "ground-up audit of every autograd-graph-recording call site "
        "that touches a LoRA factor's param.data, or (c) a different "
        "mitigation (e.g., disabling PEFT's autocast_adapter_dtype so "
        "LoRA factors stay in bf16 and the autocast cast is a no-op "
        "that records no _to_copy op). All three fall outside the "
        "M6C-fix-4 MAY-edit scope (chunk/manager.py, runtime/scheduler.py "
        "only). Tracked for a follow-up."
    ),
)
...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "M6C-fix-{1,2,3,4} now cover every M6C runtime gather path we "
        "can identify (cross-mode resume hook, profiler container gather, "
        "runtime container hooks, synchronous Scheduler routing). All "
        "verified at single-GPU + multi-rank gloo unit scope. The 4-rank "
        "Phase 1 (Mode C train+save) still fails at iter-0 backward with "
        "the canonical 'ToCopyBackward0 ... shape compatible with [0]'. "
        "M6C-fix-4 was empirically NOT attempted against this direction "
        "per the safety protocol (one multi-GPU attempt per direction "
        "max; the A→C verification surfaced the same persistent failure "
        "after the fix). Closing this likely requires a torch anomaly-"
        "mode trace from a 4-rank live run to pinpoint the exact "
        "autograd op-construction site whose source tensor is the "
        "[0] empty placeholder, or an unrelated mitigation such as "
        "disabling PEFT's autocast_adapter_dtype so the LoRA factor "
        "weights stay in bf16 (eliminating the _to_copy op the failure "
        "fires through). Tracked for a follow-up."
    ),
)
