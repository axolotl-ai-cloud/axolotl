...
_BLACKWELL = torch.cuda.is_available() and torch.cuda.get_device_capability()[0] in (10, 12)
pytestmark = pytest.mark.skipif(not _BLACKWELL, reason="grouped fp4 needs sm100/sm120 (Blackwell)")
...
_CFG = dict(name="gemma4", E=32, H=2816, I=704, topk=8, N=512, r=16,
            act_type="gelu_tanh", limit=1e30, scaling=2.0)
...
def make_routing(N: int, E: int, top_k: int, skew: str, seed: int = 0):
    ...
    if skew == "balanced":
        ...
        for i in range(N):
            rows.append(hot.clone() if (i % 10) else torch.randperm(E, generator=g)[:top_k])
        # flip: 90% hot, 10% random
        rows = [hot.clone() if (i % 10) else torch.randperm(E, generator=g)[:top_k] for i in range(N)]
    elif skew == "moderate":
    ...
    wts = torch.softmax(torch.randn(N, top_k, generator=g), -1).to(torch.bfloat16).to(DEV)
    return idx, wts
...
def _build(cfg, skew, seed=0):
    ...
    mk = lambda *s: (torch.randn(*s, device=DEV, dtype=torch.bfloat16) * 0.02).requires_grad_(True)
...
def _run(cfg, gu_nv, dn_nv, hidden, idx, wts, lora, cache):
    ...
    from axolotl.integrations.kernels.libs.scattermoe_lora.grouped_train import grouped_fp4_moe_train
    Agu, Bgu, Adn, Bdn = lora
    s = cfg["scaling"]
    return grouped_fp4_moe_train(hidden, idx, wts, gu_nv, dn_nv, (Agu, Bgu, s), (Adn, Bdn, s),
...
def test_correctness_across_skew(skew):
    from axolotl.integrations.kernels.libs.scattermoe_lora.grouped_train import grouped_fp4_available
    ...
    Agu, Bgu, Adn, Bdn = lora
    out = _run(_CFG, gu_nv, dn_nv, hidden.clone(), idx, wts,
    ...
    ref = _bf16_oracle(hidden.clone(), idx, wts, Wgu_b, Wdn_b, Agu, Bgu, Adn, Bdn,
                       _CFG["act_type"], _CFG["limit"], _CFG["scaling"])
    cos = _cos(out, ref)
...
def _peak_transient(cfg, skew):
    ...
    det = tuple(p.detach() for p in lora); cache = {}
    o = _run(cfg, gu_nv, dn_nv, hidden.clone(), idx, wts, det, cache); del o  # warmup + build cache
...
def test_no_oom_all_to_one_under_cap():
    ...
    frac = min(0.5, (needed * 12 + 4 * 2**30) / total)  # robust needs ~12x routed + model; blowup needs ~Ex more
    torch.cuda.set_per_process_memory_fraction(frac, 0)
    try:
        out = _run(_CFG, gu_nv, dn_nv, hidden.clone(), idx, wts, tuple(p.detach() for p in lora), {})
...
def test_speed_skew_within_factor_of_balanced():
    ...
    tb = timed("balanced"); ts = timed("extreme")
