...
The `activation_offloading: hidden_states` mode is gradient checkpointing that offloads only
the per-layer input (the checkpoint's `hidden_states`) to CPU — one tensor per layer rather than every
activation — overlapped with compute on a side CUDA stream. By default it keeps Transformers'
normal non-reentrant checkpointing path (`use_reentrant: false`) and filters the saved-tensor hook
to tensors marked at decoder-layer checkpoint entry, so non-checkpointed intermediates, masks,
and output-head tensors stay on GPU.

Set `gradient_checkpointing_kwargs.use_reentrant: true` to use the ALST-style reentrant
implementation instead. That path replaces torch's reentrant `CheckpointFunction` and is
DTensor-aware for sequence/context parallelism.
