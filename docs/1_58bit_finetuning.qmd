...
[`qat: weight_dtype: ternary`](./qat.qmd#ternary-qat) trains the same scheme natively, with no extra dependency.
Prefer it when taking a high precision model down to ternary; prefer `use_onebitllms` when the checkpoint is
already ternary and you want its `quantize_to_1bit` export.
