...
::: {.callout-note}
Sample packing is supported for multimodal runs — see [Multipack](multipack.qmd). Unlike the config above, packing requires a prepared dataset, so leave `skip_prepare_dataset` unset (its `length` column and cached media are what the packer needs); keep `remove_unused_columns: false`. Packing auto-uses the balanced-greedy knapsack for multimodal. Cross-attention models (e.g. mllama) are not supported.
:::
