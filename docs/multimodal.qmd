...
- [Qwen3.5](#sec-qwen3-5)
- [Qwen3.8-Flash-Next](#sec-qwen3-8-flash-next)
- [GLM-4.6V](#sec-glm-4-6v)
- [SmolVLM2](#sec-smolvlm2)
- [LFM2-VL](#sec-lfm2-vl)
- [Intern-VL](#sec-intern-vl)
- [PaddleOCR-VL](#sec-paddleocr-vl)
- [North Micro Vision Instruct](#sec-north-micro-vision-instruct)
- [Muse Glimmer](#sec-muse-glimmer)
...
Please see [examples](https://github.com/axolotl-ai-cloud/axolotl/tree/main/examples) folder for full configs.
...
::: {.callout-tip}
Please make sure to install `torchvision` via `uv pip install torchvision`.
:::
...
See the [example configs and tips](https://docs.axolotl.ai/docs/models/qwen3.8-flash-next.html) for details.
...
Keep `sample_packing: false` and do not enable Liger or Cut Cross Entropy. See the [example configs and tips](https://docs.axolotl.ai/docs/models/paddleocr-vl.html) for details.
...
| Qwen2-VL / Qwen2.5-VL / Qwen3-VL | `<\|image_pad\|>` | Axolotl wraps bare placeholders with `<\|vision_start\|>` / `<\|vision_end\|>` at encode time (the processor does **not** add them, and mrope needs them) |
| Gemma-3 | `<start_of_image>` | Processor expands to 256 `<image_soft_token>` |
| Gemma-4 | `<\|image\|>` | Processor expands to 280 image tokens (flanked by boi/eoi markers) |
...
pretraining_dataset:
  - path: /path/to/shards/*.jsonl
    ...
    # skip_bad_images: false           # drop malformed/unloadable rows instead of aborting
    # allow_remote_images: false       # permit http(s)/URL image sources (off = SSRF-safe)
    # max_images_per_row: 32           # reject/drop rows with more images (0 = unlimited)
...
`test_datasets` accepts multimodal entries (`type: multimodal_pretrain` or `multimodal: true`). Every multimodal eval entry requires `path` and an explicit `split` — omitting `split` would silently evaluate on the train split. Per-entry `text_column` and `image_column` are honored independently. When more than one multimodal entry is provided, `image_base_dir`, `image_token`, `skip_bad_images`, and `allow_remote_images` must be either unset on every entry or identical across them, because the eval collator resolves them once for the merged eval stream.
...
### Bad rows and image sources

By default a row that fails validation (placeholder/image-count mismatch, over-length, too many images, an image that won't load, or an image that loads but crashes the processor — e.g. extreme aspect ratios on Qwen) aborts the run — failures are loud, not silent. Set `skip_bad_images: true` on the dataset entry to warn-and-drop those rows instead, which is useful for large, noisy corpora.

Rows carrying more than `max_images_per_row` images (default 32) are rejected (or dropped with `skip_bad_images`) — each image expands to a large pixel tensor at the collator, so an unbounded count is an OOM vector. Set `max_images_per_row: 0` to disable the cap.

Image sources with a URL scheme (`http://`, `https://`, …) are **rejected by default** — a training collator fetching arbitrary URLs from dataset shards is an SSRF vector. Set `allow_remote_images: true` to opt in. When `image_base_dir` is set it acts as a sandbox: any path — relative or absolute, including via symlinks — that resolves outside it is rejected.
...
For this multimodal CPT path, the collator masks image-family token ids to
`-100` in the labels tensor. The masked set is built per-processor from a
static list (`<image>`, `<\|image_pad\|>`, `<\|vision_start\|>`,
`<\|vision_end\|>`, `<start_of_image>`, `<end_of_image>`, `<image_soft_token>`,
`<\|image\|>`, …) **plus** any registered special token the processor inserts
for image layout — tile/grid/global markers such as SmolVLM/Idefics
`<fake_token_around_image>`, `<global-img>`, and `<row_i_col_j>`. Detection is
restricted to special/added tokens, so real text and `bos`/`eos`/`pad` are
never masked.
