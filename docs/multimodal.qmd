...
By default a row that fails validation (placeholder/image-count mismatch,
over-length, or an image that won't load) aborts the run — failures are loud,
not silent. Set `skip_bad_images: true` on the dataset entry to warn-and-drop
those rows instead, which is useful for large, noisy corpora.

Image sources with a URL scheme (`http://`, `https://`, …) are **rejected by
default** — a training collator fetching arbitrary URLs from dataset shards is
an SSRF vector. Set `allow_remote_images: true` to opt in. Relative paths that
escape `image_base_dir` (e.g. `../../etc/passwd`) are always rejected.
