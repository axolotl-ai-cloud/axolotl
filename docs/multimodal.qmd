...
sample_packing: false  # packing is supported; see the note below
...
::: {.callout-note}
Sample packing is supported for multimodal runs — see [Multipack](multipack.qmd). To pack, set `sample_packing: true` and keep `remove_unused_columns: false`. Leaving `skip_prepare_dataset` unset uses the default prepared path (globally-optimal packing on a cached `length` column and media — recommended); setting `skip_prepare_dataset: true` (or `streaming: true`) instead routes to the buffered packer, which tokenizes and packs on the fly (non-deterministic, single-process). Packing auto-uses the balanced-greedy knapsack for multimodal. Cross-attention models (e.g. mllama) are not supported.
:::
