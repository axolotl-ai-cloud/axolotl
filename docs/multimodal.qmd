...
- [North Micro Vision Instruct](#sec-north-micro-vision-instruct) *(NEW)*
...
Axolotl now supports Cohere Labs' [North Micro Vision Instruct](https://docs.axolotl.ai/docs/models/cohere-north-micro-vision-instruct.html), which uses the `cohere_compass` architecture.

```yaml
base_model: CohereLabs/North-Micro-Vision-instruct-preview
processor_type: AutoProcessor
```

Requires `transformers>=5.15.0`. Keep `sample_packing: false` for image data. Cut Cross Entropy is supported and recommended. See the [example configs and tips](https://docs.axolotl.ai/docs/models/cohere-north-micro-vision-instruct.html) for details.
