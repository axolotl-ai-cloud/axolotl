...
::: {.callout-note}
Qwen3.5-2B is text-only. All larger variants (7B and above) are early-fusion VLMs
(`Qwen3_5ForConditionalGeneration`) — vision and text tokens share the same transformer
layers. Axolotl always loads the full VLM class regardless of whether images are present
in the dataset.
:::

```yaml
base_model: Qwen/Qwen3.5-9B  # smallest VLM variant
...
**Multimodal FFT (image + text, single 80 GB GPU)**

Measured peak VRAM — Qwen3.5-9B, `adamw_bnb_8bit`, `sequence_len: 4096`:

| Metric | Value |
|---|---|
| Max active | 60.83 GiB |
| Device reserved | 62.43 GiB |

See [examples/qwen3.5/9b-fft-vision.yaml](https://github.com/axolotl-ai/axolotl/blob/main/examples/qwen3.5/9b-fft-vision.yaml).
For 27B multimodal FFT across multiple GPUs, see [examples/qwen3.5/27b-fft-vision.yaml](https://github.com/axolotl-ai/axolotl/blob/main/examples/qwen3.5/27b-fft-vision.yaml).
