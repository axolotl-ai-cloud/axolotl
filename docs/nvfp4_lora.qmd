...
ModelOpt NVFP4 checkpoints (e.g. [nvidia/Qwen3-30B-A3B-NVFP4](https://huggingface.co/nvidia/Qwen3-30B-A3B-NVFP4)) store weights as packed 4-bit FP4 with per-block scales. Axolotl trains LoRA adapters directly on these checkpoints: the base stays 4-bit-packed in memory while the LoRA `A`/`B` matrices train in bf16. A 30B-A3B model trains in ~30 GiB at `sequence_len: 2048`.

Two expert kernels support the NVFP4 expert path:

| Kernel | Config | Compute | Hardware |
|---|---|---|---|
| SonicMoE | `use_sonicmoe: true` | W4A4 native (W4A16 fallback) | Blackwell SM100+ (W4A4), Hopper (W4A16) |
