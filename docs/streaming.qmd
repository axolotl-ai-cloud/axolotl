...
incrementally rather than loading the entire dataset into memory at once. This is
particularly useful for large pretraining datasets that don't fit in memory, or in
other situations where you don't want to pay the up-front cost of dataset preprocessing.
...
- You're doing pretraining with massive text corpora
- You're working with datasets from the HuggingFace Hub or stored locally
...
to consider:
- The `streaming_multipack_buffer_size` affects memory usage
- Sample packing requires buffering multiple samples
- Shuffling requires additional memory for the shuffle buffer

### Performance

- Streaming may have slightly higher latency compared to preprocessed datasets
- Network speed matters when streaming from remote sources
