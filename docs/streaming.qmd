...
packed into a single sequence to maximize GPU utilization. Multimodal (VLM) streaming packing is also supported via a buffered packer (`mm_pack_buffer_size`); it is single-process and non-deterministic across runs — see [Multipack](multipack.qmd).
