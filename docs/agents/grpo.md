...
| SCOPE-RL | `scope_rl: true` (plugin) | Hold entropy at a target instead of collapsing |
...
colder. Ignored for multimodal batches. Ships as a plugin; the `scope_*` keys are top-level.
...
Both marked lines are required and rejected if missing. Leave `trainer_cls` unset; the plugin sets it. Whole groups are resampled and never
...
src/axolotl/
  ...
  integrations/scope_rl/           # SCOPE-RL plugin — entropy-controlled auxiliary rollouts
