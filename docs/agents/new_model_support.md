...
- `quantizers` maps quantization-method names to `QuantizerRegistration(quantizer_cls, config_cls)`. Registration is idempotent (re-applying the same classes is a no-op); a different existing registration is overwritten with a warning. A custom quantizer can hook the weight-conversion pipeline via `HfQuantizer.update_weight_conversions` — and that is where checkpoint-conditional conversions belong (e.g. mixed-precision checkpoints whose experts are NVFP4 under a shared-FP8 method): the `weight_conversions` field registers unconditionally for the `model_type`, while the quantizer's hook only runs when a quantizer is active, giving the quantized-vs-plain conditionality for free. A `BEFORE_MODEL_BUILD` hook that inspects the model config is the fallback for conditionality the quantizer cannot express.
