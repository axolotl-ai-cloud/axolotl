...
- **Baseline perf:** stacking `torch_compile: true` measured ~−16% steady-state on Qwen3-8B + LoRA r=16 + torch 2.11 + sm_86 (RTX 3090 Ti), and is a net win on sm_120 (Blackwell).
- **Compile cold-start:** adds 5–15 min on the first step.
- **Tuning:** `torch_compile_options: {coordinate_descent_tuning: true}` adds a further ~0.5% steady-state by enabling Inductor's per-kernel block-size search, but adds another 10–30 s per unique kernel signature on first compile — the ROI only matters for long-running jobs.
