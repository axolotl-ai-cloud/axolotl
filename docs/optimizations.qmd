...
- **Tuning (`torch_compile_options`):** an opt-in allowlist of `torch._inductor.config` knobs — `coordinate_descent_tuning`, `coordinate_descent_check_all_directions`, `shape_padding`, `epilogue_fusion`, `max_autotune_gemm`, `fx_graph_cache`, `assume_aligned_inputs`, `comprehensive_padding`, `decompose_mem_bound_mm`, `triton.cudagraphs`. Payoff is arch-specific: on **sm_120 + ≥32 GB**, `max_autotune_gemm: true` adds ~−2.6% over plain compile (the only flag measured to help, and best run alone); the same flag **regresses ~+13% on sm_86**. `coordinate_descent_tuning` and the rest are within seed noise on every card tested. `triton.cudagraphs` needs static shapes — incompatible with `sample_packing`.
