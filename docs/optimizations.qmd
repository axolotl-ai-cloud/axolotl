...
- **Config:** `liger_kernel_impl: cutile | cutedsl` optionally selects an alternative kernel backend (liger-kernel >= 0.8.1, off by default): `cutile` (`pip install cuda-tile`) covers cross_entropy/geglu/layer_norm/rope and more; `cutedsl` (`pip install nvidia-cutlass-dsl`) covers cross_entropy/rms_norm, tuned for Blackwell. `fused_linear_cross_entropy` stays on Triton with either backend.
...
### Vision Patch Embed as GEMM (Qwen2-VL / Qwen2.5-VL / Qwen2.5-Omni / Qwen3-VL(-MoE) / Qwen3.5(-MoE) / Qwen3-Omni-MoE / GLM-4V(-MoE) / GLM-OCR)

Runs the vision tower's patchify `Conv3d` as its exact `F.linear` equivalent — `kernel_size == stride` with no padding makes them the same GEMM, the way vLLM implements patchify. Enabled by default for the supported families; numerically identical up to bf16 reduction order. The GEMM is ~11x faster than cuDNN's conv3d for this shape (1.4 ms vs 16 ms per 3 MP image), and it sidesteps PyTorch's `slow_conv_dilated3d` reference fallback (~41 s per image) that eager Conv3d hits on torch builds without cuDNN — the eager evaluation path is where that cliff otherwise shows up, since `torch.compile` already avoids it during training.

```yaml
vision_patch_embed_linear: false  # opt out; the default (unset) is on
```

- **Checkpoint-neutral:** weights stay in the Conv3d module, so state_dicts, LoRA targeting, and `modules_to_save` are unchanged in both directions.
- **Automatic fallback to the stock forward** when PEFT wraps `patch_embed.proj` itself, under DeepSpeed ZeRO-3 (partitioned placeholder weights), or when Accelerate offload hooks manage the projection — those paths need the module call.
