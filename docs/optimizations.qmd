...
- **Hardware note:** on sm_120 (Blackwell) combining with `torch_compile: true` is a net win; on sm_86 (Ampere consumer) `torch_compile: true` currently regresses the surrounding Inductor-generated kernels — keep compile off there.
...
Trains LoRA adapters on a ModelOpt NVFP4 MoE checkpoint with the experts kept 4-bit-packed in memory (W4A4 grouped GEMM on the frozen base; LoRA `A` / `B` train in bf16).
The native NVFP4 tensor-core path runs on Blackwell (B200/GB200, SM100+); other GPUs fall back to a dequant-to-bf16 path.

- **Config:** `use_sonicmoe: true` with a ModelOpt NVFP4 `base_model` (e.g. `nvidia/Qwen3-30B-A3B-NVFP4`)
- **Example:** [Qwen3-30B-A3B NVFP4 LoRA](https://github.com/axolotl-ai-cloud/axolotl/blob/main/examples/qwen3/30b-a3b-nvfp4-lora.yaml)
- **Learn more:** [SonicMoE NVFP4 backend](custom_integrations.qmd#kernels-integration)
