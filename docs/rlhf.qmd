...
To run the DPO loss through liger-kernel's fused (chunked) implementation, set `dpo_use_liger_kernel: true`. The fused path supports `dpo_loss_type` values `sigmoid` (default), `hinge`, `exo_pair`, `nca_pair`, `robust`, `bco_pair`, `sppo_hard`, `apo_zero`, `apo_down`, and `discopop` (liger-kernel >= 0.8.1); `ipo` is not supported on the fused path, and only the first entry of `dpo_loss_type` is used.
