...
First, launch a `vLLM` server using `trl vllm_serve` - you may use a config file or CLI overrides to configure your vLLM server. In this example, we're
...
```bash
CUDA_VISIBLE_DEVICES=2,3 axolotl vllm_serve vllm_config.yaml
```

Your `vLLM` instance will now attempt to spin up, and it's time to kick off training. To configure the `GRPOTrainer` to use the vLLM server,
...
Finally, let's kick off training utilizing our remaining two GPUs:
