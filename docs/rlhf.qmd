...
trl:
    ...
    gdpo_decoupled_norm: true   # Enable per-reward normalization (default: true)
    gdpo_batch_norm: false      # Optional batch normalization (default: false)
    gdpo_epsilon: 1e-4          # Numerical stability constant
...
| Aspect | GRPO | GDPO |
|--------|------|------|
| **Reward Normalization** | Combine first, then normalize | Normalize each, then combine |
...
|-----------|------|---------|-------------|
| `gdpo_decoupled_norm` | bool | `true` | Enable per-reward normalization |
| `gdpo_batch_norm` | bool | `false` | Apply batch normalization after combining |
| `gdpo_epsilon` | float | `1e-4` | Numerical stability constant |
| `gdpo_per_reward_scale` | bool | `true` | Scale each reward by its std |
