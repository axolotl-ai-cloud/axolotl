...
### Async Generation Options (`trl:` section)

These enable the async data pipeline where generation happens in a background thread while training proceeds on the previous batch.

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `use_data_producer` | bool | `false` | Enable the async data producer protocol |
| `async_prefetch` | bool | `false` | Generate rollouts in a background thread during training |
| `prefetch_depth` | int | `None` | Number of rollouts to prefetch ahead |
| `streaming_partial_batch` | bool | `None` | Score prompt groups incrementally instead of full batch |
| `streaming_min_groups` | int | `None` | Minimum prompt groups per streaming chunk |

### Off-Policy Correction Options (`trl:` section)

These handle distribution mismatch when vLLM weights are stale relative to the training model.

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `vllm_importance_sampling_correction` | bool | `None` | Apply IS correction for distribution mismatch |
| `importance_sampling_level` | str | `None` | `token` or `sequence` level IS ratios |
| `off_policy_mask_threshold` | float | `None` | KL threshold for off-policy sequence masking |
| `vllm_importance_sampling_mode` | str | `None` | IS mode: `token_truncate`, `token_mask`, `sequence_truncate`, or `sequence_mask` |
| `vllm_importance_sampling_cap` | float | `None` | Cap for IS ratio clipping/masking |
| `use_bias_correction_kl` | bool | `None` | Apply IS correction to KL divergence term |

## Complete Example {#sec-complete-example}

Below is a full config for GRPO training with vLLM, LoRA sync, and async generation.
