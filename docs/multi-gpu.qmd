...
When using `auto_wrap_policy: SIZE_BASED_WRAP`, set `min_num_params` to the minimum parameter count a module must have to be wrapped (e.g. `100000000`). If omitted, the size-based policy is ignored and no modules are wrapped.
