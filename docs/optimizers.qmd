...
::: {.callout-important}
Install the fork **from source** — `pip install gefen` pulls *upstream* Gefen,
which lacks this fork's modern-arch memory fallback, fused kernels, and
`GefenMuonHybrid`. Fused CUDA kernels JIT-build on first step (needs `nvcc`) and
fall back to pure-PyTorch if the build fails.
