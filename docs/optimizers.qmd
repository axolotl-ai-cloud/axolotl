...
AdamW with the weight decay corrected for the learning rate schedule. AdamW's decoupled
decay makes the steady-state gradient-to-weight-norm ratio depend on the current learning
rate, so a decaying schedule pushes the gradient norm up over the second half of training.
AdamC scales the decay by the schedule position (`lambda_t = lambda * lr_t / lr_max`),
which holds that ratio constant and keeps weight norms stable.

Paper: [https://arxiv.org/abs/2506.02285](https://arxiv.org/abs/2506.02285)
...
The peak learning rate is captured per parameter group when the optimizer is built, so
`embedding_lr`, `embedding_lr_scale`, and `lr_groups` are each corrected against their own
peak. Override it explicitly if you build a schedule that exceeds the configured LR:
...
Only groups with a non-zero weight decay are affected — axolotl already excludes
embeddings, the LM head, norms, and biases from decay, matching the paper's rule of
applying the correction to normalized layers only.
...
SCAO (Sparse Curvature-Aware Adaptive Optimizer) is a second-order optimizer
that approximates curvature information with sparse preconditioning while
keeping usage close to AdamW.

GitHub: [https://github.com/whispering3/scao](https://github.com/whispering3/scao)

Install: `pip install axolotl[optimizers]`

```yaml
optimizer: scao

# optional SCAO args can be passed through optim_args
optim_args:
  precond_freq: 20
  k_min: 8
  k_max: 128
...
off; FSDP2 (`fsdp_config:`) is supported.
