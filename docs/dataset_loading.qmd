...
To use an S3-compatible object store like [MinIO](https://min.io/), [Tigris](https://www.tigrisdata.com), or [Cloudflare R2](https://developers.cloudflare.com/r2/), keep the `s3://` prefix on your dataset path and set `AWS_ENDPOINT_URL` to the store's endpoint. `s3fs` picks this up via `boto3`, so no code change is required.
...
Tigris access key IDs are prefixed with `tid_` and secret access keys with `tsec_`. Tigris uses a single global endpoint and doesn't charge egress fees, which matters when the same dataset is read across many training epochs or from workers in multiple regions.
