...
The following legacy shapes keep working with a deprecation warning at config
load; they will be removed in a future release:

- `adapter: qlora` — normalized to `adapter: lora`. QLoRA is "LoRA with a
  4-bit base quant" expressed via `load_in_4bit: true` or
  `model_quantization_config: {bnb: {weight_dtype: nf4}}`. Bare
  `adapter: qlora` (no explicit flag) is treated as `adapter: lora` +
  `load_in_4bit: true`.
- `model_quantization_config: Mxfp4Config` (string) with
  `model_quantization_config_kwargs: …` — prefer
  `model_quantization_config: {mxfp4: {config_kwargs: …}}`.
- `model_quantization_config: FineGrainedFP8Config` (string) — prefer
  `model_quantization_config: {fp8: {config_kwargs: …}}`.
