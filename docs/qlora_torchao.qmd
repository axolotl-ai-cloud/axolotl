...
- `adapter: qlora` (with or without `load_in_4bit: true`) → `adapter: lora`
  + `model_quantization_config: {bnb: {weight_dtype: nf4}}`. QLoRA is just
  "LoRA with a 4-bit base quant"; there is no separate adapter type.
- `load_in_4bit: true` (alone) → `model_quantization_config:
  {bnb: {weight_dtype: nf4}}`.
- `load_in_8bit: true` (alone) → `model_quantization_config:
  {bnb: {weight_dtype: int8}}`.
- `model_quantization_config: Mxfp4Config` (string) with
  `model_quantization_config_kwargs: …` → `model_quantization_config:
  {mxfp4: {config_kwargs: …}}`.
- `model_quantization_config: FineGrainedFP8Config` (string) →
