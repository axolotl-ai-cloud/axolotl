...
::: {.callout-note}
FP8 support is experimental and requires compatible NVIDIA Hopper or Blackwell hardware (for example, H100, H200, B200, and supported consumer Blackwell devices) plus recent PyTorch and TorchAO versions.
:::

### What is FP8? {#sec-fp8-overview}

FP8 (8-bit floating point) can provide significant time savings compared to FP16/BF16 while maintaining training stability. Axolotl's implementation uses PyTorch's TorchAO library and supports tensorwise and rowwise scaling recipes.

### Requirements {#sec-fp8-software}

- Compatible NVIDIA Hopper or Blackwell GPUs (for example, H100, H200, B200, and supported consumer Blackwell devices; availability depends on the installed PyTorch, TorchAO, and CUDA build)
...
# Optional: Select tensorwise (default), rowwise, or rowwise_with_gw_hp scaling
...
::: {.callout-warning}
`fp8_enable_fsdp_float8_all_gather: true` currently supports only the default `tensorwise` recipe. Disable it when using `rowwise` or `rowwise_with_gw_hp`.
:::
