...
- **FP8** - 8-bit floating point (compatible Hopper or Blackwell GPUs)
...
### Master weights under FSDP2 {#sec-fp32-master-weights}

BF16 mixed precision keeps an fp32 *master* copy of each trainable
parameter and casts down only for compute. Without it, at a typical SFT
learning rate of `5e-6`, Adam steps land below bf16's mantissa resolution
and round away to nothing.

```{.yaml}
fsdp_version: 2
fp32_master_weights: true    # default for full fine-tunes
```

Defaults to `true` for full fine-tunes and `false` for adapters (PEFT
already keeps LoRA params in fp32). Set it to `false` for pure bf16: this
saves ~6 bytes per parameter per rank — weights plus the two Adam moments
— at the cost of the rounding above.

fp32 master weights also mean fp32 optimizer state and fp32 checkpoints,
roughly twice the size of a bf16 save. FSDP1 and DeepSpeed already behave
this way; accelerate applies the upcast for them itself.
...
::: {.callout-note}
FP8 support is experimental and requires compatible NVIDIA Hopper or Blackwell hardware (including supported consumer Blackwell devices) and recent PyTorch versions with TorchAO.
:::

### What is FP8? {#sec-fp8-overview}

FP8 (8-bit floating point) can provide significant time savings compared to FP16/BF16 while maintaining training stability. Axolotl's implementation uses PyTorch's TorchAO library and supports `tensorwise`, `rowwise`, and `rowwise_with_gw_hp` scaling recipes.

### Requirements {#sec-fp8-software}

- Compatible NVIDIA Hopper or Blackwell GPUs (including supported consumer Blackwell devices)
...
# Optional: Select the FP8 scaling recipe (tensorwise by default)
fp8_config:
  recipe: tensorwise
...
- **For compatible Hopper or Blackwell GPUs**: Try FP8 + torch.compile for maximum speed
