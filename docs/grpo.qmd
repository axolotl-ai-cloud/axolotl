...
- `reinforce_plus_plus`: group mean/std normalization without importance-sampling correction.
...
| `advantage_estimator` | str | `null` | Advantage baseline: `grpo` (group mean/std), `rloo` (leave-one-out), or `reinforce_plus_plus` (group mean/std, no IS correction) |
