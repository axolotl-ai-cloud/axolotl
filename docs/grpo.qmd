...
After rewards are aggregated into a per-completion scalar, the `advantage_estimator` controls how the group baseline is subtracted to form advantages. This is orthogonal to `loss_type` (which controls the policy-gradient loss formulation, not the baseline).

- `grpo` (default, set via the absence of the field): subtract the group mean and, when `scale_rewards` is on, divide by the group standard deviation.
- `rloo`: leave-one-out baseline — each completion's advantage is `reward_j - mean(rewards of the other completions in the group)`. Unbiased and avoids a completion being part of its own baseline. Requires `num_generations >= 2`.
- `reinforce_plus_plus`: global batch-level advantage normalization — `(reward - batch_mean) / (batch_std + eps)` over the whole generation batch rather than per group (REINFORCE++). Any KL penalty should be folded into the reward upstream.
...
Supported by the standard, async, and sequence-parallel GRPO trainers.
...
| `advantage_estimator` | str | `null` | Advantage baseline: `grpo` (group mean/std), `rloo` (leave-one-out), or `reinforce_plus_plus` (global batch mean/std) |
