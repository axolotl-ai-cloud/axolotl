...
GRPO entropy usually falls monotonically until the policy stops exploring. [SCOPE-RL](https://arxiv.org/abs/2510.08141) holds it at a target instead: each rollout resamples a small fraction of the prompt groups at an adjusted temperature, keeps only the completions that were rewarded, and adds them to the loss as a positive-advantage term. It ships as a plugin:
...
Both marked lines are required and rejected if missing. The plugin supplies its own trainer, so leave `trainer_cls` unset. See the [plugin reference](custom_integrations.qmd#scope-rl-entropy-control) for every option. The auxiliary rollout is only issued on the prefetching producer's path, and the token-normalised loss types (`bnpo`, `cispo`, `dapo`, `luspo`) weight rows by completion length, which would pull the auxiliary term off `scope_alpha`.

The auxiliary temperature is `clip(1 + H0 - H, 0.8, 1.2)` times your sampling temperature, where `H` is the entropy measured on the previous step: below target it samples hotter (positive samples at high temperature push entropy back up), above target it samples colder. Only positive samples are kept -- the paper's ablations show that mixing in negatives re-collapses entropy.

Cost is roughly `scope_alpha` extra generation -- under 2% at the default, but only when a rollout batch holds at least `1 / scope_alpha` groups (64 at the default). Whole groups are resampled and never fewer than one, so 4 groups per batch costs 25% whatever `scope_alpha` says. Only generation cost is affected; the loss weight stays correct.

Watch `scope/temperature` and `scope/positive_frac`: a `positive_frac` pinned at 0 means `scope_positive_threshold` is above what your reward functions actually emit, and the branch contributes nothing. The auxiliary term is averaged over every resampled row, so its effective strength is `scope_alpha * positive_frac`.

Higher entropy is not monotonically better -- the paper finds `H0 = 0.5` beats both 0.25 and 1.0. Tune `scope_target_entropy` before `scope_alpha`.
