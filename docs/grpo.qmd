...
Both marked lines are required and rejected if missing. The auxiliary rollout is only issued on the prefetching producer's path, and the token-normalised loss types (`bnpo`, `cispo`, `dapo`, `luspo`) weight rows by completion length, which would pull the auxiliary term off `scope_alpha`.
