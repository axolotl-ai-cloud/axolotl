...
```yaml
trl:
  use_data_producer: true         # SCOPE-RL requires the async trainer
  scope_rl: true
...
Cost is roughly `scope_alpha` extra generation, so under 2% at the default. Watch `scope/temperature` and `scope/positive_frac`: a `positive_frac` pinned at 0 means `scope_positive_threshold` is above what your reward functions actually emit, and the branch does nothing.
