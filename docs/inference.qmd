...
::: {.callout-note}
Whatever is shown, the *stored* conversation always keeps the model output
verbatim, thinking included. When the conversation is re-rendered each turn,
the chat template decides how prior-turn reasoning is handled — matching what
the model saw during training. The KV cache is re-used across turns whenever
the rendered conversation extends the previous one, so long chats stay
responsive.
:::
