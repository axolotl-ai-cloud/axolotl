...
Axolotl supports 3 kind of training methods: pre-training, supervised fine-tuning, and preference tuning. Each method has their own dataset format which are described below.

## [Pre-training](pretraining.qmd)

When aiming to train on large corpuses of text datasets, pre-training is your go-to choice. As these datasets are large, Axolotl enables streaming for these datasets to only load batches into memory at a time.

A sample file for a pre-training dataset is as follows:
...
Axolotl supports loading from a Huggingface hub repo or from local files.

::: {.callout-important}
For pre-training only, Axolotl would split texts if it exceeds the context length into multiple smaller prompts.
:::

### Pre-training load from Huggingface hub

To load from Huggingface dataset `hf_org/name`, you can pass the following config:

```yaml
pretraining_dataset: hf_org/name
```

### Pre-training load from local files
...
One benefit of this is that: the tokenization can be performed separately on a CPU-only machine then transferred to a GPU machine for training to save costs.
...
### Before running pre-train

#### Setting max_steps

As the dataset is streamed, Axolotl does not know in advance how large the dataset is and does not know when to stop.
...
It is recommended to leave this off if downloading from Huggingface hub as it would download the entire dataset which can be very large.

## Supervised fine-tuning

Supervised fine-tuning is the process of training models to respond to an instruction or chat input.

As there are a wide variety of dataset formats, Axolotl tries to support or opens up the option to use a majority of the formats available in public datasets.

Axolotl provide four various approaches for loading datasets, however, it's easier to work backwards from the dataset you have available to figure out which approach to use.
...
This approach is when you want to bring your own tokenized dataset.
...
Don't forget to add BOS/EOS and proper masking for it!
...
This approach is if you want granular control over the prompt formatting, special tokens, and masking while letting Axolotl handle the tokenization. This is very useful if your dataset have unique prompts that differ within, where one single general template wouldn't suffice for.

In the example below, you could see that there is no proper structure. In the same time, it's very flexible as there are no constraints on how your prompt can look.
...
::: {.callout-tip}
Fun fact: Axolotl synonymously call "chat" messages as `conversation` messages due to how FastChat initially used this term to build a widely used method [fastchat conversation](https://github.com/lm-sys/FastChat/blob/main/fastchat/conversation.py) for formatting chat messages prior to the creation of `chat_templates`.
:::

#### What are `chat_templates`?

The current most popular and convenient method for inference is to use `chat_templates` for formatting prompts. Therefore, Axolotl supports using `chat_templates` for training to ensure that the model performs in the same environment as in inference.
...
2. What are the keys in your dataset, and what are the possible roles? For example, in OpenAI format, the keys would be `messages`, `role`, `content` respectively, whereas the possible roles are `system`, `user`, and `assistant`.
3. What do you want to mask? For instance, only assistant messages, only last message, or nothing.

##### Choosing `chat_template`

There are a lot of `chat_templates` out there.

Axolotl support the common ones: (https://github.com/axolotl-ai-cloud/axolotl/blob/860609392184cf62a7e0ca676658b170e059ce6c/src/axolotl/utils/chat_templates.py#L17). For example, to use ChatML, it would be `chat_template: chatml`.

However, it is also possible to use the already configured template within the tokenizer by specifying `chat_template: tokenizer_default`. If you want a fallback (in case some tokenizer does not have it pre-configured), you can do `chat_template: tokenizer_default_fallback_chatml` to use the tokenizer or fallback to ChatML.
...
The common use case for `chat_template` is for chat messages, therefore, it is common to mask all non-assistant messages. Assistant messages refer to the bot messages that you want the model to learn as.
...
The second config means that it would mask all EOS tokens for turns that aren't assistant-turns. The other options are: `all` and `last` to choose which EOS to train on.
...
Once all the above are completed, you would combine all these configs together to form your dataset config. The last part would be to set the EOS token in the config to ensure that the code knows what your EOS token is.
...
The first number refers to the label, the second refers to the token_id. For example, `-100` labels appears on non-assistant portions, meaning that it's masked. For assistant portions, the label is the same as the token_id.

Ref: https://axolotl-ai-cloud.github.io/axolotl/docs/dataset-formats/conversation.html#chat_template

### [Instruction Dataset](inst_tune.qmd)

An Instruction dataset is a kind of dataset which focuses on instruction and output, a single turn instead of multi-turn conversations.
