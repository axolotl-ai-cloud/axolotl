...
Activations are quantized to int8 with a per-token absmax scale, norms and the LM head stay in high precision,
and `quantize_embedding` uses int8 per-row rather than ternary. `group_size` does not apply.
...
qat:
  ...
  activation_dtype: int8        # optional, the only supported activation dtype for ternary
