...
## Enabling Swap for FSDP

If available memory is insufficient even after FSDP's CPU offloading, you can enable swap memory usage by setting `cpu_offload_pin_memory: false` in `fsdp_config`.

This disables memory pinning -- necessary for FSDP to use NVMe/SSD swap space. While even slower than RAM from initial CPU offload, it unlocks a new pool of memory which may enable training that would otherwise fail with OOM errors.
