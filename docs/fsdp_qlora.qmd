...
When FSDP's CPU offloading still exceeds your available RAM, you can enable swap memory usage to offload to disk by setting AXOLOTL_DISABLE_MEMORY_PINNING=1.

This disables memory pinning -- necessary for FSDP to use NVMe/SSD swap space. While even slower than RAM, it may enable training that would otherwise fail with OOM errors.
