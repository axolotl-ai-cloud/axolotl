...
::: {.callout-important}
### Images are now uv-based

All images now use [uv](https://docs.astral.sh/uv/) with a relocatable venv (`/workspace/axolotl-venv`)
instead of Miniconda + pip. The plain names (e.g. `axolotlai/axolotl`) and the `-uv` names
(e.g. `axolotlai/axolotl-uv`) now serve the **same uv image**. The `-uv` names are kept as aliases, so
existing references to either keep working. Tags follow the same format for both.
:::
