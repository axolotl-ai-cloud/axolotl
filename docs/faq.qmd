...
**Q: How to know the value to use for `fsdp_config.transformer_layer_cls_to_wrap`?**
...
> A: We currently recommend torch 2.12 for use with `vllm`. Please ensure you use the right version. For Docker, please use a `-vllm` tagged image such as `main-py3.12-cu130-2.12.1-vllm`.
...
**Q: How do I run an external benchmark or eval during training?**

> A: Use the [Benchmark API plugin](custom_integrations.qmd) (`axolotl.integrations.benchmark_api.BenchmarkAPIPlugin`). By default it POSTs on save events to an HTTP endpoint you run; set `run_on` to also (or instead) trigger on evaluation or train end. It logs the returned scalar metrics back into training, with optional early stopping and asynchronous (non-blocking) execution.
...
> A: This is because the tokenizer does not have a chat template. Please add a chat template in the tokenizer config. See [chat_template](dataset-formats/conversation.qmd#chat_template) for more details.
