...
> A: Use the [Benchmark API plugin](custom_integrations.qmd) (`axolotl.integrations.benchmark_api.BenchmarkAPIPlugin`). It POSTs each checkpoint to an HTTP endpoint you run and logs the returned scalar metrics back into training, with optional early stopping and async (non-blocking) runners.
