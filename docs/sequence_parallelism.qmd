...
4. Attention runs through ringmaster (Ulysses all-to-all, ring, or USP — auto-selected
   from the KV-head count and topology), wrapping the model's own attention kernel
...
- torch >= 2.11
- The `ringmaster` package. Install with:
  - `pip install axolotl[ringmaster]` (preferred)
  - `pip install axolotl-ringmaster`

## Limitations

- A kernel-backed attention implementation is required (`sdpa`, `flash_attention_2/3`,
  or `flex_attention`); `eager` attention is not supported
- `sample_packing` is not yet supported with context parallelism
