...
<!-- LOCAL DRAFT, not pushed. Tested 2026-09-24 on 0.19.0 unless noted (job IDs in the Work vault note):
- First run (Gemma 4 E2B, a10g-large), curl + flags, bucket config + checkpoints, model-repo config, merge chain, 4-GPU ZeRO-2, fineweb-edu streaming: all passed.
- main-latest works with `hf jobs run` (no cuda13_env needed).
- Gemma 4 E2B on 0.19.0 hung on a10g-small (RUNNING, no logs) 4 times; fine on a10g-large, and on a10g-small with main-latest. Hence a10g-large on this page.
- Resume from a bucket checkpoint is untested; the page does not claim it.
-->
...
- `--name gemma4-e2b-trial` labels the Job, so you can find it again later with `hf jobs ps --name gemma4-e2b-trial`. Names do not have to be unique.
...
Any top-level config key can also be passed as a flag after the config path, with underscores written as dashes, for example `axolotl train /configs/train.yml --max-steps 5`. Flags override the file, which is handy for a quick change, but the YAML stays the record of what ran.
...
```bash
hf buckets create your-username/axolotl-runs
...
```bash
hf repos create your-username/gemma4-e2b-lora --private
...
hf jobs run --flavor a10g-large --timeout 1h -s HF_TOKEN \
  -v ./configs:/configs \
  axolotlai/axolotl:0.19.0-py3.12-cu130-2.12.1 -- bash -c '
    axolotl train /configs/train.yml &&
