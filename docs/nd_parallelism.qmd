...
| DDP + TP/CP | >1 | 1 | >1 | >1 | ❌ **Not Supported**. The `ParallelismConfig` explicitly prevents this, as composing pure DDP with TP is currently not supported. You should use FSDP + TP instead (`dp_shard_size > 1`). |
