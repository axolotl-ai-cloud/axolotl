...
Axolotl enables training large models via composable parallelism methods, primarily leveraging Pytorch's `FSDP` and `DeviceMesh`. This is essential when:
...
- Distributed Data Parallel (DDP): The classic approach. The full model is replicated on every GPU. Each GPU processes a different slice of the data batch. Gradients are averaged across all GPUs (`all_reduce`) after the backward pass to keep the models synchronized. This is simple but memory-inefficient, as each GPU must hold the entire model, its gradients, and optimizer states.
...
This matrix describes how different parallelism methods can be combined in Axolotl using `DeviceMesh`.
...
Axolotl will construct the appropriate `DeviceMesh` for PyTorch depending on the following configs.
...
2.  FSDP + TP on a single 8-GPU node:
    ...
    - Set `dp_shard_size: 4`, `tensor_parallel_size: 2`, `dp_replicate_size: 1`.
