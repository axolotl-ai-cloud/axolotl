...
Telemetry must be **decided upon** before using axolotl. To enable it, set
`AXOLOTL_DO_NOT_TRACK=0` or `DO_NOT_TRACK=0`. To disable it, set 
...
The software will exit without collecting any data if a decision is not made.

To hide the warning message about telemetry that is displayed on train, etc. startup,
and use the software explicitly set: `AXOLOTL_DO_NOT_TRACK=0` (enable telemetry) 
