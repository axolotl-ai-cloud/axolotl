...
### Launchers and Runtime Configs

Commands that support multi-GPU (`train`, `evaluate`, ...) take a `--launcher`
(`accelerate` (default), `torchrun`, `ray` (train only), or `python`) and an
optional `--runtime` file describing the cluster (see the
[runtime config reference](runtime-config.qmd)):

```bash
# Reusable cluster description; also selects the launcher
axolotl train config.yml --runtime cluster.yaml

# Train on all GPUs of a Ray cluster (or submit via the Ray Jobs API)
axolotl train config.yml --launcher ray
```

Launcher-specific arguments can also be passed directly using the `--` separator;
they override matching values from the runtime file:
...
# Train via Ray (in-cluster, or Ray Jobs submission when the runtime file
# points at http://head:8265)
axolotl train config.yml --runtime cluster.yaml
...
Manages a Ray cluster for `--launcher ray` training (see the
[Ray integration docs](ray-integration.qmd)).

```bash
# On the head node: start a head and join hostfile workers over ssh
axolotl ray up --hostfile hostfile

# Cluster summary (nodes, CPUs/GPUs)
axolotl ray status

# Tear down head + workers
axolotl ray down
```
