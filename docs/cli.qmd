...
Exports a trained model to GGUF for llama.cpp, Ollama, and LM Studio.

```bash
axolotl export config.yml --quantize Q4_K_M
```

See [Exporting to GGUF](./export.qmd) for more details.
...
## Remote Compute with Cloud Launchers

Axolotl supports remote training through Modal, Baseten, Nebius, and installed cloud launcher plugins.
Configure the provider in a cloud YAML file alongside your regular Axolotl config. Install the Modal SDK
with `pip install 'axolotl[modal]'`; Baseten uses `pip install 'axolotl[baseten]'`.

The cloud YAML's `provider` selects the launcher: `modal` (the default), `baseten`, `nebius`,
or an installed [cloud launcher plugin](custom_integrations.qmd#adding-a-cloud-launcher).
Use `image: registry/image:tag` for a prebuilt runtime, or `image_build` with a
Dockerfile and local context to deploy a fork and custom plugins. See
[image configuration](custom_integrations.qmd#selecting-or-building-the-runtime-image).

Training, preprocessing, and lm-eval all use this selection; providers report
unsupported operations explicitly. Baseten and [Nebius Serverless Jobs](nebius-serverless.qmd) currently support training only. Set training overrides directly in the
training YAML for Baseten; CLI config overrides are rejected.
...
```yaml
provider: modal # selects the Modal launcher
gpu:         # GPU type to use
