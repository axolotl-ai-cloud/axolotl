...
For improved training stability, you can enable reward centering which encourages the model to produce mean-zero outputs by adding an auxiliary loss:

```yaml
base_model: google/gemma-2-2b

reward_model: true
center_rewards_coefficient: 0.01  # Typical values: 0.001 - 0.1
chat_template: gemma
datasets:
  - path: argilla/distilabel-intel-orca-dpo-pairs
    type: bradley_terry.chat_template
...
The `center_rewards_coefficient` parameter controls the weight of the auxiliary loss that minimizes the squared sum of rewards. This technique, proposed by [Eisenstein et al., 2023](https://huggingface.co/papers/2312.09244) (Eq. 2), helps ensure the reward model naturally produces mean-zero outputs without requiring post-training normalization.
