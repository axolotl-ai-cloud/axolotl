...
Building `flash-attn` from source requires a CUDA toolkit (`nvcc`) and an
Ampere+ GPU; restrict the build to your architecture (e.g.
`TORCH_CUDA_ARCH_LIST=8.0` for A100) to keep compile time and memory in check.
