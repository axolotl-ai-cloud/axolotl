...
GPU="${GPU:-0}"
# active venv by default; override with --venv or VENV=
VENV="${VENV:-${VIRTUAL_ENV:-.venv}}"
CFG=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --gpu) GPU="$2"; shift 2;;
...
sed "s#^output_dir:.*#output_dir: $OUT#; s/saves_per_epoch: .*//" "$CFG" > "$WORK/run1.yaml"
grep -q "max_steps:" "$WORK/run1.yaml" && sed -i "s/max_steps: .*/max_steps: 8/" "$WORK/run1.yaml" \
  || echo "max_steps: 8" >> "$WORK/run1.yaml"
grep -q "save_strategy:" "$WORK/run1.yaml" && sed -i "s/save_strategy: .*/save_strategy: steps/" "$WORK/run1.yaml" \
...
# accept either a LoRA adapter or a full-model save (FFT writes no adapter)
saved = any(os.path.isfile(os.path.join(ck, f)) for f in
            ("adapter_model.safetensors", "model.safetensors", "model.safetensors.index.json"))
ok = os.path.isdir(ck) and saved
print(f"checkpoint-4 saved: {ok}")
gs = None
